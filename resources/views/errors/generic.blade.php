...
<head>
    ...
    <title>{{ $statusCode }} - {{ $title }} | {{ config('app.name') }}</title>
    ...
    <section class="hero-section px-4 py-16">
        <div class="max-w-xl mx-auto text-center">
            <div class="inline-flex items-center gap-2 px-4 py-2 bg-slate-600 rounded-full shadow mb-6">
                ...
                <span class="text-white font-bold text-sm">{{ __('errors.http_generic_title') }}</span>
            </div>
            <h1 class="text-6xl md:text-7xl font-black text-gray-900 mb-2">{{ $statusCode }}</h1>
            <h2 class="text-2xl font-bold text-gray-800 mb-4">{{ $title }}</h2>
            <p class="text-lg text-gray-600 leading-relaxed mb-10">{{ $message }}</p>
            <div class="flex flex-col sm:flex-row gap-4 justify-center">
                <a href="{{ $errorHomeUrl }}" class="inline-flex items-center justify-center gap-2 bg-slate-700 text-white px-8 py-3 rounded-full font-bold shadow-lg hover:bg-slate-800 transition-colors">
