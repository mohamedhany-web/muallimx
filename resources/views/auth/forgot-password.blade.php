...
<body class="bg-white">
    <div class="flex min-h-screen lg:h-screen">
        ...
        {{-- لوحة بصرية (نفس أسلوب تسجيل الدخول / الصفحة الرئيسية) --}}
        ...
        <div class="flex-1 flex flex-col items-center justify-center px-5 sm:px-8 py-10 lg:py-0 bg-white relative overflow-y-auto">
            ...
            <div class="lg:hidden w-full max-w-md">
                @include('partials.auth-brand-link', ['size' => 'sm', 'fallback' => 'gradient', 'mb' => 'mb-8'])
            </div>
            ...
            <div class="w-full max-w-md">
                <div class="text-center lg:text-{{ $isRtl?'right':'left' }} mb-8">
                    ...
                    <p class="text-slate-500 text-sm sm:text-base">{{ __('auth.forgot_password_help') }}</p>
                </div>
                ...
                <form method="POST" action="{{ route('password.email') }}" class="space-y-5">
                    @csrf
                    <div>
                        <label for="email" class="block text-sm font-bold text-navy-950 mb-2">{{ __('auth.email') }}</label>
                        <div class="relative">
                ...
                <div class="mt-8 pt-6 border-t border-slate-100 text-center">
                    <a href="{{ route('login') }}" class="inline-flex items-center gap-2 text-sm font-bold text-[#283593] hover:text-[#1F2A7A] transition-colors">
