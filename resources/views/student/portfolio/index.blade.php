...
<div class="w-full px-4 sm:px-6 lg:px-8 py-6 space-y-6">
    ...
    <div class="bg-white dark:bg-slate-800/95 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden">
        ...
        <div class="p-5 grid grid-cols-1 md:grid-cols-2 gap-3">
            @foreach($marketingCapabilities as $cap)
                <div class="rounded-xl border px-4 py-3 {{ $cap['active'] ? 'border-emerald-200 bg-emerald-50/70 dark:border-emerald-800/60 dark:bg-emerald-900/20' : 'border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800/40' }}">
                    <div class="flex items-center justify-between gap-3">
                        <p class="text-sm font-semibold {{ $cap['active'] ? 'text-emerald-800 dark:text-emerald-200' : 'text-slate-700 dark:text-slate-300' }}">{{ $cap['label'] }}</p>
                        <span class="text-[11px] px-2 py-1 rounded-lg font-semibold {{ $cap['active'] ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300' : 'bg-slate-200 text-slate-600 dark:bg-slate-700 dark:text-slate-300' }}">
                            {{ $cap['active'] ? 'مفعلة' : 'غير مفعلة' }}
                        </span>
                    </div>
                </div>
            @endforeach
