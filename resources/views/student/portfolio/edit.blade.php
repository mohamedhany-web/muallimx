...
@section('title', 'تعديل عنصر - البورتفوليو')
@section('header', 'تعديل عنصر')
        ...
        <form action="{{ route('student.portfolio.update', $project) }}" method="POST" enctype="multipart/form-data" class="p-6 md:p-8" x-data="{ type: {{ json_encode(old('content_type', $project->content_type ?? 'gallery')) }} }">
            ...
            <div class="mb-6">
                ...
                <textarea name="description" rows="3"
                          class="w-full rounded-xl border-2 border-[#2CA9BD]/20 px-4 py-3 focus:border-[#2CA9BD] focus:ring-2 focus:ring-[#2CA9BD]/20">{{ old('description', $project->description) }}</textarea>
            </div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                        @foreach($project->images as $img)
                            <div class="rounded-2xl overflow-hidden border border-gray-200 bg-gray-50">
                                <a href="{{ asset($img->image_path) }}" target="_blank" class="block">
                                    <img src="{{ asset($img->image_path) }}" alt="image" class="w-full h-28 object-cover">
                                </a>
                                <div class="p-2">
                                    <form action="{{ route('student.portfolio.images.destroy', [$project, $img]) }}" method="POST" onsubmit="return confirm('حذف الصورة؟');">
                                        @csrf
                                        @method('DELETE')
