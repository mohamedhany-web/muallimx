...
@section('title', 'تفاصيل المشروع - البورتفوليو')
@section('header', 'تفاصيل المشروع')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="space-y-2 text-sm">
                        <div class="flex items-center justify-between gap-3">
                            ...
                            @if($project->project_url)
                                <a href="{{ $project->project_url }}" target="_blank" class="font-bold text-sky-700 dark:text-sky-300 hover:underline">فتح</a>
                            @else
                        ...
                        <div class="flex items-center justify-between gap-3">
                            ...
                            @if($project->video_url)
                                <a href="{{ $project->video_url }}" target="_blank" class="font-bold text-sky-700 dark:text-sky-300 hover:underline">فتح</a>
                            @else
                                <span class="text-slate-400">—</span>
                            @endif
                        </div>
                ...
                <div class="rounded-2xl border border-slate-200 dark:border-slate-700 p-4">
                    ...
                    <div class="space-y-1 text-sm text-slate-700 dark:text-slate-200">
                        <p><span class="font-bold">المراجع:</span> {{ $project->reviewer?->name ?? '—' }}</p>
                        <p><span class="font-bold">تاريخ المراجعة:</span> {{ $project->reviewed_at ? $project->reviewed_at->format('Y-m-d H:i') : '—' }}</p>
                        <p><span class="font-bold">تاريخ النشر:</span> {{ $project->published_at ? $project->published_at->format('Y-m-d H:i') : '—' }}</p>
                    </div>
                </div>
            </div>
                ...
                @else
                    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                        @foreach($project->images as $img)
                            <a href="{{ asset($img->image_path) }}" target="_blank" class="block rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800/40">
                                <img src="{{ asset($img->image_path) }}" alt="image" class="w-full h-28 object-cover">
                            </a>
                        @endforeach
                    </div>
                @endif
