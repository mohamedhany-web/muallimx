...
<body class="bg-white text-navy-950 antialiased font-body">
    ...
    <main class="flex-1">
        ...
        <section class="relative overflow-hidden bg-navy-950 noise pt-24 pb-20 md:pb-28">
            ...
            <div class="absolute bottom-[-10%] {{ $isRtl?'right-[-5%]':'left-[-5%]' }} w-[400px] h-[400px] rounded-full bg-purple-600/8 blur-[100px]"></div>
            ...
            <div class="relative z-10 max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 w-full">
                ...
                <nav class="reveal text-sm text-slate-400/80 mb-10 flex items-center gap-2 flex-wrap">
                    ...
                    <a href="{{ route('public.instructors.index') }}" class="hover:text-white transition-colors">{{ __('public.instructors_page_title') }}</a>
                    <i class="fas fa-chevron-{{ $isRtl?'left':'right' }} text-[8px] text-slate-600"></i>
                    <span class="text-white/90 font-medium">{{ $profile->user->name }}</span>
                </nav>
                    ...
                    <div class="reveal stagger-1 flex-1 min-w-0">
                        ...
                        <div class="flex flex-wrap gap-3 mb-6">
                            ...
                            <div class="flex items-center gap-2.5 px-4 py-2.5 rounded-xl bg-white/[0.06] border border-white/[0.08] text-white/90 text-sm font-medium">
                                ...
                                <span>{{ count($profile->skills_list) }} مهارة</span>
                            </div>
                            @endif
                        ...
                        <div class="flex flex-wrap gap-2.5">
                            ...
                            <a href="{{ $socials['linkedin'] }}" target="_blank" rel="noopener noreferrer" class="w-11 h-11 rounded-xl bg-[#0A66C2]/90 text-white flex items-center justify-center shadow-lg hover:bg-[#004182] hover:scale-110 transition-all">
                                <i class="fab fa-linkedin-in text-lg"></i>
                            </a>
                            @endif
                            @if(!empty($socials['twitter']))
                            <a href="{{ $socials['twitter'] }}" target="_blank" rel="noopener noreferrer" class="w-11 h-11 rounded-xl bg-navy-800 text-white flex items-center justify-center shadow-lg hover:bg-navy-900 hover:scale-110 transition-all border border-white/10">
                                <i class="fab fa-x-twitter text-lg"></i>
                            </a>
                            @endif
                            @if(!empty($socials['youtube']))
                            <a href="{{ $socials['youtube'] }}" target="_blank" rel="noopener noreferrer" class="w-11 h-11 rounded-xl bg-red-600/90 text-white flex items-center justify-center shadow-lg hover:bg-red-700 hover:scale-110 transition-all">
                                <i class="fab fa-youtube text-lg"></i>
                            </a>
                            @endif
                            @if(!empty($socials['facebook']))
                            <a href="{{ $socials['facebook'] }}" target="_blank" rel="noopener noreferrer" class="w-11 h-11 rounded-xl bg-[#1877F2]/90 text-white flex items-center justify-center shadow-lg hover:bg-[#0d65d9] hover:scale-110 transition-all">
                                <i class="fab fa-facebook-f text-lg"></i>
                            </a>
                            @endif
                            @if(!empty($socials['website']))
                            <a href="{{ $socials['website'] }}" target="_blank" rel="noopener noreferrer" class="w-11 h-11 rounded-xl bg-white/10 text-white flex items-center justify-center shadow-lg hover:bg-white/20 hover:scale-110 transition-all border border-white/10">
                    ...
                    <div class="lg:col-span-2 space-y-8">
                        ...
                        <div class="reveal stagger-1 card-hover rounded-3xl bg-white border border-slate-100 p-6 sm:p-8 shadow-sm hover:shadow-xl hover:border-amber-200/50">
                            <div class="flex items-center gap-3 mb-5">
                                ...
                                <h2 class="font-heading text-2xl font-black text-navy-950">{{ __('public.experience') }}</h2>
                            </div>
                            @if(count($profile->experience_list) > 0)
                            <div class="space-y-3">
                        ...
                        <div class="reveal stagger-2 card-hover rounded-3xl bg-white border border-slate-100 p-6 sm:p-8 shadow-sm hover:shadow-xl hover:border-blue-200/50">
                            <div class="flex items-center gap-3 mb-5">
                                ...
                                <h2 class="font-heading text-2xl font-black text-navy-950">{{ __('public.instructor_courses') }}</h2>
                            </div>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                @foreach($courses as $c)
                                @php $cThumb = $c->thumbnail ? str_replace('\\','/', $c->thumbnail) : null; @endphp
                                <a href="{{ route('public.course.show', $c->id) }}" class="group flex gap-4 p-4 rounded-2xl border border-slate-100 hover:border-brand-200 hover:shadow-lg transition-all duration-300 bg-white">
                                    <div class="w-20 h-20 flex-shrink-0 rounded-xl bg-gradient-to-br from-brand-500 to-navy-600 overflow-hidden flex items-center justify-center">
                                        @if($cThumb)
                                            <img src="{{ asset('storage/' . $cThumb) }}" alt="" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                                        @else
                                            <i class="fas fa-book text-white/80 text-2xl"></i>
                                        @endif
                                    </div>
                                    <div class="flex-1 min-w-0 flex flex-col justify-center">
                                        <h4 class="font-bold text-navy-950 group-hover:text-brand-600 transition-colors line-clamp-2 leading-snug text-sm mb-1.5">{{ $c->title }}</h4>
                                        <div class="flex items-center gap-3 text-xs text-slate-500">
                                            @if($c->price > 0)
                                                <span class="font-bold text-brand-600">{{ number_format($c->price, 0) }} {{ __('public.currency_egp') }}</span>
                                            @else
                    ...
                    <div class="lg:col-span-1">
                        <div class="reveal sticky top-24 space-y-6">
                            ...
                            <div class="card-hover rounded-3xl bg-white border border-slate-100 shadow-lg overflow-hidden">
                                <div class="bg-gradient-to-{{ $isRtl?'r':'l' }} from-purple-500 to-purple-600 p-5">
                                ...
                                <div class="p-5">
                                    <div class="flex flex-wrap gap-2">
                                        ...
                                        <span class="px-3 py-1.5 rounded-xl bg-gradient-to-br from-purple-50 to-slate-50 text-slate-700 text-sm font-medium border border-purple-100/60">{{ $skill }}</span>
                                        @endforeach
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="card-hover rounded-3xl bg-white border border-slate-100 shadow-lg overflow-hidden">
                                ...
                                <div class="p-5 space-y-3">
                                    <div class="flex justify-between items-center p-3 bg-slate-50/80 rounded-xl text-sm">
                                        ...
                                        <span class="font-bold text-navy-950">{{ $courses->count() }}</span>
                                    </div>
                                    <div class="flex justify-between items-center p-3 bg-slate-50/80 rounded-xl text-sm">
                                        <span class="text-slate-500 flex items-center gap-2"><i class="fas fa-cogs text-purple-500"></i> المهارات</span>
                                        <span class="font-bold text-navy-950">{{ count($profile->skills_list) }}</span>
                                    </div>
                                    <div class="flex justify-between items-center p-3 bg-slate-50/80 rounded-xl text-sm">
                                ...
                                <div class="px-5 pb-5">
                                    <a href="{{ route('public.courses') }}" class="btn-primary block w-full text-center py-3 rounded-2xl bg-gradient-to-l from-brand-500 to-brand-600 text-white font-bold shadow-lg text-sm">
                            ...
                            {{-- Back button --}}
                            <a href="{{ route('public.instructors.index') }}" class="btn-outline flex items-center justify-center gap-2.5 w-full bg-white border-2 border-slate-200 hover:border-brand-300 text-navy-950 font-semibold py-3.5 rounded-2xl text-sm">
