...
@section('title', 'تعديل صفحة هبوط')
@section('header', 'تعديل صفحة هبوط')
@section('content')
@php
...
<div class="w-full space-y-6">
    <div class="rounded-3xl bg-white/95 backdrop-blur border border-slate-200 shadow-lg overflow-hidden">
        <div class="px-5 py-6 sm:px-8 border-b border-slate-200 flex flex-wrap items-center justify-between gap-3">
            <div>
                <h1 class="text-xl font-bold text-slate-900">تعديل: {{ $landingPage->title }}</h1>
                <p class="text-slate-500 mt-1 flex flex-wrap items-center gap-2">
                    <code class="text-xs bg-slate-100 px-2 py-1 rounded" dir="ltr">/lp/{{ $landingPage->slug }}</code>
                    <a href="{{ $landingPage->publicUrl() }}" target="_blank" class="text-sky-600 text-sm font-semibold hover:underline">فتح الصفحة العامة</a>
                </p>
            </div>
            <a href="{{ route('admin.landing-pages.index') }}" class="text-sm font-semibold text-slate-600 hover:text-sky-600">← العودة للقائمة</a>
        </div>
        <form action="{{ route('admin.landing-pages.update', $landingPage) }}" method="POST" enctype="multipart/form-data" class="p-5 sm:p-8 space-y-6">
            @csrf
            @method('PUT')
            @include('admin.landing-pages._form', ['sectionsJson' => $sectionsJson ?? [], 'landingPage' => $landingPage])
