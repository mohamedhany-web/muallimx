...
@section('title', 'إضافة ملف جديد - ' . config('app.name', 'Muallimx'))
...
<div class="min-h-screen bg-gray-50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <form action="{{ route('admin.media.store') }}" method="POST" enctype="multipart/form-data" class="bg-white shadow rounded-lg p-6">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-md bg-white text-gray-900">{{ old('description') }}</textarea>
                    @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
