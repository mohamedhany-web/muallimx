...
@if(isset($hiringAcademies) && $hiringAcademies->count())
<div class="rounded-xl border border-indigo-100 bg-indigo-50/50 dark:bg-indigo-950/20 dark:border-indigo-900 p-4 mb-2">
    <label class="block text-xs font-bold text-indigo-800 dark:text-indigo-200 mb-1">{{ __('admin.academy_opportunity_hiring_academy') }}</label>
    <select name="hiring_academy_id" class="w-full px-3 py-2 rounded-lg border border-indigo-200 dark:border-indigo-800 dark:bg-slate-800 dark:text-white">
        ...
        @foreach($hiringAcademies as $ha)
            <option value="{{ $ha->id }}" @selected(old('hiring_academy_id', $opportunity?->hiring_academy_id) == $ha->id)>{{ $ha->name }} @if($ha->city) — {{ $ha->city }} @endif</option>
        @endforeach
    </select>
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <div>
        ...
        <input type="date" name="apply_until" value="{{ old('apply_until', optional($opportunity?->apply_until)->format('Y-m-d')) }}" class="w-full px-3 py-2 rounded-lg border border-slate-200">
    </div>
