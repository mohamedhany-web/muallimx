...
<div class="space-y-6">
    ...
    <div class="rounded-2xl bg-white border border-slate-200 shadow-sm p-6">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('admin.students-control.paid-features') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-slate-200 text-slate-700 hover:bg-slate-50 text-sm font-bold">
                    <i class="fas fa-layer-group"></i>
                    إدارة المزايا المدفوعة
                </a>
                <a href="{{ route('admin.portfolio.index') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-slate-200 text-slate-700 hover:bg-slate-50 text-sm font-bold">
    ...
    <div class="flex flex-wrap gap-2">
        <a href="{{ route('admin.portfolio-marketing-profiles.index', ['status' => 'pending_review']) }}" class="px-4 py-2 rounded-xl text-sm font-bold {{ $status === 'pending_review' ? 'bg-amber-600 text-white' : 'bg-gray-200 text-gray-700' }}">
            قيد المراجعة @if($pendingCount > 0)<span class="mr-1 opacity-90">({{ $pendingCount }})</span>@endif
        </a>
        <a href="{{ route('admin.portfolio-marketing-profiles.index', ['status' => 'rejected']) }}" class="px-4 py-2 rounded-xl text-sm font-bold {{ $status === 'rejected' ? 'bg-red-600 text-white' : 'bg-gray-200 text-gray-700' }}">مرفوض</a>
        <a href="{{ route('admin.portfolio-marketing-profiles.index', ['status' => 'approved']) }}" class="px-4 py-2 rounded-xl text-sm font-bold {{ $status === 'approved' ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-700' }}">معتمد</a>
        <a href="{{ route('admin.portfolio-marketing-profiles.index', ['status' => 'all']) }}" class="px-4 py-2 rounded-xl text-sm font-bold {{ $status === 'all' ? 'bg-slate-700 text-white' : 'bg-gray-200 text-gray-700' }}">الكل</a>
    </div>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($users as $u)
                        <tr class="hover:bg-gray-50/80">
                            <td class="px-4 py-3 text-sm font-semibold text-gray-900">{{ $u->name }}</td>
                            ...
                            <td class="px-4 py-3 text-sm text-gray-600">{{ $u->portfolio_profile_submitted_at?->format('Y-m-d H:i') ?? '—' }}</td>
                            <td class="px-4 py-3 text-sm">
                                <a href="{{ route('admin.portfolio-marketing-profiles.show', $u) }}" class="text-sky-600 font-bold hover:underline">عرض ومراجعة</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
