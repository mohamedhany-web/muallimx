    ...
    <section class="rounded-3xl bg-white/95 backdrop-blur border border-slate-200 shadow-lg overflow-hidden">
        <div class="px-5 py-6 sm:px-8 border-b border-slate-200">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
                ...
                <a href="{{ route('admin.accounting.reports.export', array_merge(request()->query(), ['type' => 'orders'])) }}" class="inline-flex items-center gap-2 rounded-2xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-emerald-700">
                    <i class="fas fa-file-excel"></i>
                    تصدير Excel
                </a>
            </div>
            <form method="GET" action="{{ route('admin.accounting.reports.orders') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
            ...
            <p class="text-sm text-slate-500 mt-3">من {{ $startDate->format('Y-m-d') }} إلى {{ $endDate->format('Y-m-d') }} — إجمالي الطلبات: {{ $stats['order_stats']['total_orders'] }} — معتمدة: {{ $stats['order_stats']['approved_orders'] }}</p>
        </div>
