...
<div class="space-y-6">
    ...
    <form method="GET" class="bg-white dark:bg-slate-800 rounded-xl p-4 border border-slate-200 dark:border-slate-700 flex flex-wrap gap-3 items-end">
        <div class="flex-1 min-w-[220px]">
            ...
            <input type="text" name="search" value="{{ $search }}" placeholder="عنوان، كود، اسم المعلم، أو البريد..." class="w-full rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white text-sm">
        </div>
        <div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                @forelse($meetings as $meeting)
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                        ...
                        <td class="px-4 py-3">
                            <p class="font-medium text-slate-800 dark:text-white">{{ $meeting->title ?: ('اجتماع #' . $meeting->id) }}</p>
        ...
        @if($meetings->hasPages())
            <div class="px-4 py-3 border-t border-slate-200 dark:border-slate-700">{{ $meetings->links() }}</div>
        @endif
    </div>
</div>
@endsection
