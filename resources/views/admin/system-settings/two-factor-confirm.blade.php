    ...
    <section class="rounded-2xl border border-slate-200 dark:border-slate-600 bg-gradient-to-br from-slate-50 via-white to-violet-50/40 dark:from-slate-800 dark:via-slate-800 dark:to-slate-900 p-6 sm:p-8 shadow-sm">
        <div class="flex flex-col sm:flex-row sm:items-start gap-5">
            ...
            <div class="flex-1 min-w-0 space-y-2">
                ...
                @if($userEmail)
                    <p class="text-xs font-semibold text-slate-500 dark:text-slate-400" dir="ltr">{{ $userEmail }}</p>
                @endif
            </div>
        </div>
    </section>
    ...
    <div class="rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 shadow-lg overflow-hidden p-6 sm:p-8 space-y-6">
        <form method="post" action="{{ route('admin.system-settings.two-factor.confirm.submit') }}" class="space-y-5">
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <a href="{{ route('admin.system-settings.edit') }}" class="inline-flex items-center gap-2 px-6 py-3 rounded-xl border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-200 text-sm font-bold hover:bg-slate-50 dark:hover:bg-slate-700">
                    إلغاء والعودة
                </a>
            </div>
        </form>
