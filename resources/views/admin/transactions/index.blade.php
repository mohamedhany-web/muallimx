...
<div class="space-y-6">
    ...
    <section class="rounded-2xl bg-white border border-slate-200 shadow-lg overflow-hidden">
        <div class="px-4 py-4 bg-slate-50 border-b border-slate-200 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <a href="{{ route('admin.transactions.create') }}" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-gradient-to-r from-blue-600 to-blue-500 rounded-xl shadow hover:from-blue-700 hover:to-blue-600 transition-all">
    ...
    <section class="rounded-2xl bg-white border border-slate-200 shadow-lg overflow-hidden">
        ...
        <div class="p-4">
            <form method="GET" action="{{ route('admin.transactions.index') }}" id="filterForm" class="flex flex-col gap-3 sm:flex-row sm:items-end sm:flex-wrap">
                <div class="flex-1 min-w-[180px]">
                    <label class="block text-xs font-semibold text-slate-700 mb-1">البحث</label>
                    <input type="text" name="search" value="{{ request('search') }}" maxlength="255" placeholder="رقم المعاملة، اسم العميل، هاتف" class="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                </div>
                <div class="w-full sm:w-auto min-w-[140px]">
                    ...
                    <select name="type" class="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500">
                        ...
                        <option value="credit" {{ request('type') == 'credit' ? 'selected' : '' }}>إيراد</option>
                        <option value="debit" {{ request('type') == 'debit' ? 'selected' : '' }}>مصروف</option>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <a href="{{ route('admin.transactions.index') }}" class="inline-flex items-center justify-center rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50" title="مسح الفلتر">
    ...
    <section class="rounded-2xl bg-white border border-slate-200 shadow-lg overflow-hidden">
        <div class="px-4 py-3 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            ...
            <span class="text-xs font-semibold text-blue-600 bg-blue-50 px-2.5 py-1 rounded-lg border border-blue-200">{{ $transactions->total() }} معاملة</span>
        </div>
            ...
            <div class="p-3 space-y-1.5">
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
                                ...
                                <div class="flex-1 min-w-0 space-y-1">
                                    <div class="flex flex-wrap items-center justify-between gap-2">
                                        <div>
                                            <p class="text-sm font-bold text-slate-900">{{ $transaction->transaction_number ?? '—' }}</p>
                                            <p class="text-xs text-slate-600">{{ $transaction->user->name ?? '—' }} · {{ $transaction->user->phone ?? $transaction->user->email ?? '—' }}</p>
                                        </div>
                                        <div class="flex items-center gap-1.5 flex-wrap">
                                            @php $typeMeta = $typeBadges[$transaction->type] ?? ['label' => $transaction->type, 'classes' => 'bg-slate-100 text-slate-700 border border-slate-200', 'icon' => 'fas fa-circle']; @endphp
                                            <span class="inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-xs font-semibold {{ $typeMeta['classes'] }}">
                                                <i class="{{ $typeMeta['icon'] ?? 'fas fa-circle' }} text-[10px]"></i>
                                                {{ $typeMeta['label'] }}
                                            </span>
                                            @php $statusMeta = $statusBadges[$transaction->status] ?? ['label' => $transaction->status ?? '—', 'classes' => 'bg-slate-100 text-slate-700 border border-slate-200']; @endphp
                                ...
                                <div class="flex items-center gap-1.5 flex-shrink-0">
                                    <a href="{{ route('admin.transactions.show', $transaction) }}" class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-300 bg-white text-blue-600 hover:bg-blue-50 text-sm" title="عرض">
                ...
                @empty
                    <div class="rounded-xl border border-slate-200 bg-white p-10 text-center">
                        ...
                        <a href="{{ route('admin.transactions.create') }}" class="mt-4 inline-flex items-center gap-2 rounded-xl bg-blue-600 hover:bg-blue-700 px-4 py-2 text-sm font-semibold text-white transition-colors">
                            <i class="fas fa-plus"></i>
                            إضافة معاملة
                        </a>
                    </div>
