...
<div class="space-y-6">
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
            <p class="text-xs text-gray-500 font-semibold">مشرف: {{ $supervisor->name }} · مضيف: {{ $student->name }}</p>
            <h1 class="text-2xl font-bold text-gray-900">{{ $meeting->title ?: ('غرفة '.$meeting->code) }}</h1>
            <p class="text-sm text-gray-600 mt-1">كود الجلسة: <span class="font-mono font-bold">{{ $meeting->code }}</span></p>
        </div>
        <div class="flex flex-wrap gap-3">
            @if($meeting->isLive())
                <a href="{{ route('admin.academic-supervision.supervisors.meetings.observe', [$supervisor, $meeting]) }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-600 text-white text-sm font-bold hover:bg-emerald-500">مراقبة الآن</a>
            @endif
            <a href="{{ route('admin.academic-supervision.supervisors.students.show', [$supervisor, $student]) }}" class="text-sm font-semibold text-teal-700 hover:underline self-center">← رجوع</a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
            ...
            <p class="text-sm font-bold text-gray-900">{{ $participants->count() }} شخص</p>
            @if($meeting->isLive())
            ...
            <table class="min-w-full text-sm">
                ...
                <tbody class="divide-y divide-emerald-100 bg-white">
                    @foreach($presentNow as $p)
                        <tr>
                            <td class="px-4 py-2 font-medium">{{ $p->display_name ?: 'بدون اسم' }}</td>
            ...
            <table class="min-w-full text-sm">
                <thead class="bg-gray-50 text-gray-600 font-semibold">
                    <tr>
                        ...
                        <th class="text-right px-4 py-2">المدة</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    @empty
                        <tr><td colspan="6" class="px-4 py-8 text-center text-gray-500">لا يوجد سجل حضور.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
