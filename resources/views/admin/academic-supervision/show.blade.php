...
<script type="application/json" id="academic-all-students-json">@json($allStudents)</script>
<div class="space-y-6" x-data="{
    ...
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">{{ $supervisor->name }}</h1>
            <p class="text-sm text-gray-600">{{ $supervisor->email }}</p>
        </div>
        <a href="{{ route('admin.academic-supervision.index') }}" class="text-sm font-semibold text-teal-700 hover:underline">← قائمة المشرفين</a>
    </div>
    ...
    @endif
    @if(session('info'))
        <div class="rounded-xl border border-sky-200 bg-sky-50 text-sky-800 text-sm px-4 py-3">{{ session('info') }}</div>
    @endif
    @if(session('error'))
