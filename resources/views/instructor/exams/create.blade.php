...
@section('title', __('instructor.create_exam_new') . ' - Mindlytics')
...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="rounded-2xl bg-white border border-slate-200 shadow-sm p-5 sm:p-6 mb-6">
        <nav class="text-sm text-slate-500 mb-2">
            ...
            <span class="text-slate-700 font-semibold">{{ __('instructor.create_exam_new') }}</span>
        </nav>
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div class="flex flex-wrap items-center gap-4">
                <div class="w-12 h-12 rounded-xl {{ $isOfflineMode ? 'bg-violet-100 text-violet-600' : 'bg-sky-100 text-sky-600' }} flex items-center justify-center shrink-0">
                    <i class="fas fa-clipboard-list text-lg"></i>
                </div>
                <div class="min-w-0">
                    <h1 class="text-xl sm:text-2xl font-bold text-slate-800">
                        {{ $isOfflineMode ? __('instructor.create_exam_offline') : __('instructor.create_exam_new') }}
                    </h1>
                    <p class="text-sm text-slate-600 mt-0.5">
                        @if($isOfflineMode && $selectedOfflineTitle)
                            {{ __('instructor.course_label_short') }}: {{ $selectedOfflineTitle }}
                        @else
                        {{ __('instructor.add_exam_to_course') }}
                        @endif
                    </p>
                </div>
            </div>
    ...
    <div class="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
        <form action="{{ route('instructor.exams.store') }}" method="POST">
            ...
            <div class="p-6 sm:p-8 space-y-8">
                <!-- معلومات الاختبار -->
                <div class="space-y-6">
                    <h2 class="text-lg font-bold text-slate-800 border-b border-slate-200 pb-2">{{ __('instructor.exam_info') }}</h2>
                    <div>
                        <label for="title" class="block text-sm font-semibold text-slate-700 mb-1">{{ __('instructor.title') }} <span class="text-red-500">*</span></label>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('instructor.course_type') }} <span class="text-red-500">*</span></label>
                        <div class="flex flex-wrap gap-4">
                            <label class="inline-flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="course_type" value="online" {{ !$isOfflineMode ? 'checked' : '' }} class="rounded border-slate-300 text-sky-600 focus:ring-sky-500">
                                <span>{{ __('instructor.online_course') }}</span>
                            </label>
                            <label class="inline-flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="course_type" value="offline" {{ $isOfflineMode ? 'checked' : '' }} class="rounded border-slate-300 text-sky-600 focus:ring-sky-500">
                                <span>{{ __('instructor.offline_course') }}</span>
                            </label>
                        </div>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div id="online-course-wrap" class="{{ $isOfflineMode ? 'hidden' : '' }}">
                            <label for="advanced_course_id" class="block text-sm font-semibold text-slate-700 mb-1">{{ __('instructor.online_course') }} <span class="text-red-500">*</span></label>
                        ...
                        <div id="offline-course-wrap" class="{{ $isOfflineMode ? '' : 'hidden' }}">
                            <label for="offline_course_id" class="block text-sm font-semibold text-slate-700 mb-1">{{ __('instructor.offline_course') }} <span class="text-red-500">*</span></label>
                            <select name="offline_course_id" id="offline_course_id"
                                ...
                                <option value="">{{ __('instructor.choose_offline_course') }}</option>
                                @foreach($offlineCourses ?? [] as $oc)
                                    <option value="{{ $oc->id }}" {{ old('offline_course_id', request('offline_course_id')) == $oc->id ? 'selected' : '' }}>{{ $oc->title }}</option>
                                @endforeach
                            </select>
                            @error('offline_course_id')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                        </div>
                        <div id="lesson-wrap" class="{{ $isOfflineMode ? 'hidden' : '' }}">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 text-slate-800" placeholder="{{ __('instructor.description_placeholder') }}">{{ old('description') }}</textarea>
                    </div>
                    <div>
                        <label for="instructions" class="block text-sm font-semibold text-slate-700 mb-1">{{ __('instructor.exam_instructions') }}</label>
                        <textarea name="instructions" id="instructions" rows="4" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 text-slate-800" placeholder="{{ __('instructor.instructions_placeholder') }}">{{ old('instructions') }}</textarea>
                    </div>
                </div>

                <!-- التوقيت والدرجات -->
                <div class="space-y-6 pt-6 border-t border-slate-200">
                    <h2 class="text-lg font-bold text-slate-800 border-b border-slate-200 pb-2">{{ __('instructor.time_and_marks') }}</h2>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        <div>
                            <label for="duration_minutes" class="block text-sm font-semibold text-slate-700 mb-1">{{ __('instructor.duration_minutes') }} ({{ __('instructor.minute_unit') }}) <span class="text-red-500">*</span></label>
                            <input type="number" name="duration_minutes" id="duration_minutes" value="{{ old('duration_minutes', 60) }}" min="5" max="480" required
                                   class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 text-slate-800">
                        </div>
                        <div>
                            <label for="total_marks" class="block text-sm font-semibold text-slate-700 mb-1">{{ __('instructor.total_marks') }} <span class="text-red-500">*</span></label>
                            <input type="number" name="total_marks" id="total_marks" value="{{ old('total_marks', 100) }}" min="1" required
                                   class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 text-slate-800">
                        </div>
                        <div>
                            <label for="passing_marks" class="block text-sm font-semibold text-slate-700 mb-1">{{ __('instructor.passing_marks') }} <span class="text-red-500">*</span></label>
                            <input type="number" name="passing_marks" id="passing_marks" value="{{ old('passing_marks', 60) }}" min="0" step="0.5" required
                                   class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 text-slate-800">
                        </div>
                        <div>
                            <label for="attempts_allowed" class="block text-sm font-semibold text-slate-700 mb-1">{{ __('instructor.attempts_allowed') }} <span class="text-red-500">*</span></label>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            <label for="start_time" class="block text-sm font-semibold text-slate-700 mb-1">{{ __('instructor.start_time') }}</label>
                            <input type="datetime-local" name="start_time" id="start_time" value="{{ old('start_time') }}"
                                   class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 text-slate-800">
                        </div>
                        <div>
                            <label for="end_time" class="block text-sm font-semibold text-slate-700 mb-1">{{ __('instructor.end_time') }}</label>
                ...
                <div class="space-y-6 pt-6 border-t border-slate-200">
                    <h2 class="text-lg font-bold text-slate-800 border-b border-slate-200 pb-2">{{ __('instructor.display_settings') }}</h2>
                    <div class="flex flex-wrap gap-6">
                        <label class="inline-flex items-center gap-2 cursor-pointer">
                            ...
                            <span class="text-sm font-medium text-slate-700">{{ __('instructor.randomize_questions') }}</span>
                        </label>
                        <label class="inline-flex items-center gap-2 cursor-pointer">
                            ...
                            <span class="text-sm font-medium text-slate-700">{{ __('instructor.randomize_options') }}</span>
                        </label>
                        <label class="inline-flex items-center gap-2 cursor-pointer">
                            ...
                            <span class="text-sm font-medium text-slate-700">{{ __('instructor.show_results_immediately') }}</span>
                        </label>
                        <label class="inline-flex items-center gap-2 cursor-pointer">
                            ...
                            <span class="text-sm font-medium text-slate-700">{{ __('instructor.show_correct_answers') }}</span>
                        </label>
                        <label class="inline-flex items-center gap-2 cursor-pointer">
                            ...
                            <span class="text-sm font-medium text-slate-700">{{ __('instructor.show_explanations') }}</span>
                        </label>
                        <label class="inline-flex items-center gap-2 cursor-pointer">
                            ...
                            <span class="text-sm font-medium text-slate-700">{{ __('instructor.allow_review') }}</span>
                        </label>
                        <label class="inline-flex items-center gap-2 cursor-pointer">
                            ...
                            <span class="text-sm font-medium text-slate-700">{{ __('instructor.exam_active') }}</span>
                        </label>
                        <label class="inline-flex items-center gap-2 cursor-pointer">
                            ...
                            <span class="text-sm font-medium text-slate-700">{{ __('instructor.show_in_sidebar') }}</span>
                        </label>
                    </div>
                    <div class="max-w-xs">
                        <label for="sidebar_position" class="block text-sm font-semibold text-slate-700 mb-1">{{ __('instructor.sidebar_position') }}</label>
                ...
                <div class="pt-6 border-t border-slate-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <div class="flex gap-3 shrink-0">
                        ...
                        <a href="{{ route('instructor.exams.index') }}" class="px-6 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-semibold transition-colors">
                            {{ __('common.cancel') }}
                        </a>
                    </div>
                </div>
