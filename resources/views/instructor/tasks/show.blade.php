...
<div class="space-y-6">
    <div class="rounded-2xl bg-white dark:bg-slate-800/95 border border-slate-200 dark:border-slate-700 shadow-sm p-5 sm:p-6">
        <nav class="text-sm text-slate-500 dark:text-slate-400 mb-4">
            ...
            <span class="text-slate-700 dark:text-slate-300 font-semibold">{{ $task->title }}</span>
        </nav>
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <h1 class="text-xl font-bold text-slate-800 dark:text-slate-100">{{ $task->title }}</h1>
            ...
            <div class="flex items-center gap-2">
                @if(!$task->assigned_by)
                    <a href="{{ route('instructor.tasks.edit', $task) }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-slate-100 dark:bg-slate-700/50 hover:bg-slate-200 text-slate-700 dark:text-slate-300 rounded-xl font-semibold text-sm transition-colors">
                        <i class="fas fa-edit"></i>
                        {{ __('common.edit') }}
                    </a>
                @endif
                <a href="{{ route('instructor.tasks.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-sky-500 dark:bg-sky-600 hover:bg-sky-600 text-white rounded-xl font-semibold text-sm transition-colors">
    ...
    @if($task->description)
        <div class="rounded-2xl bg-white dark:bg-slate-800/95 border border-slate-200 dark:border-slate-700 shadow-sm p-5 sm:p-6">
            ...
            <p class="text-slate-600 dark:text-slate-400 whitespace-pre-wrap">{{ $task->description }}</p>
        </div>
    @endif

    <div class="rounded-2xl bg-white dark:bg-slate-800/95 border border-slate-200 dark:border-slate-700 shadow-sm p-5 sm:p-6">
        <h3 class="font-bold text-slate-800 dark:text-slate-100 mb-4">{{ __('instructor.additional_details') }}</h3>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            @if($task->relatedCourse)
                <div class="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-slate-100 dark:border-slate-700/80">
                    ...
                    <div>
                        <p class="text-xs font-semibold text-slate-500 dark:text-slate-400">{{ __('instructor.course') }}</p>
                        <p class="text-slate-800 dark:text-slate-100 font-medium">{{ $task->relatedCourse->title }}</p>
                    </div>
                </div>
            @endif
            @if($task->relatedLecture)
                <div class="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-slate-100 dark:border-slate-700/80">
                    ...
                    <div>
                        <p class="text-xs font-semibold text-slate-500 dark:text-slate-400">{{ __('instructor.lecture') }}</p>
                        <p class="text-slate-800 dark:text-slate-100 font-medium">{{ $task->relatedLecture->title }}</p>
                    </div>
                </div>
            @endif
            @if($task->due_date)
                <div class="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-slate-100 dark:border-slate-700/80">
                    ...
                    <div>
                        <p class="text-xs font-semibold text-slate-500 dark:text-slate-400">{{ __('instructor.due_date') }}</p>
                        <p class="text-slate-800 dark:text-slate-100 font-medium">{{ $task->due_date->format('Y-m-d H:i') }}</p>
            ...
            @if($task->completed_at)
                <div class="flex items-center gap-3 p-3 bg-emerald-50 dark:bg-emerald-900/30 rounded-xl border border-emerald-100">
                    ...
                    <div>
                        <p class="text-xs font-semibold text-emerald-700 dark:text-emerald-400">{{ __('instructor.completed') }}</p>
                        <p class="text-slate-800 dark:text-slate-100 font-medium">{{ $task->completed_at->format('Y-m-d H:i') }}</p>
                    </div>
                </div>
            @endif
            @if($task->assigned_by && isset($task->progress))
                <div class="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-slate-100 dark:border-slate-700/80">
                    ...
                    <div>
                        <p class="text-xs font-semibold text-slate-500 dark:text-slate-400">{{ __('instructor.progress_label') }}</p>
                        <p class="text-slate-800 dark:text-slate-100 font-medium">{{ (int)($task->progress ?? 0) }}%</p>
                    </div>
                </div>
            @endif
        </div>
    ...
    @if($task->assigned_by)
        ...
        <div class="rounded-2xl bg-white dark:bg-slate-800/95 border border-slate-200 dark:border-slate-700 shadow-sm p-5 sm:p-6">
            <h3 class="font-bold text-slate-800 dark:text-slate-100 mb-4">{{ __('instructor.update_progress') }}</h3>
            <form action="{{ route('instructor.tasks.update-progress', $task) }}" method="POST" class="flex flex-wrap items-end gap-4">
                @csrf
                @method('PUT')
                <div>
                    <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('common.status') }}</label>
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('instructor.progress_percent') }}</label>
        ...
        {{-- التسليمات --}}
        <div class="rounded-2xl bg-white dark:bg-slate-800/95 border border-slate-200 dark:border-slate-700 shadow-sm p-5 sm:p-6">
            <h3 class="font-bold text-slate-800 dark:text-slate-100 mb-4">{{ __('instructor.my_submissions') }}</h3>
                ...
                <ul class="space-y-3 mb-6">
                    @foreach($task->deliverables as $d)
                        <li class="flex flex-wrap items-start justify-between gap-3 p-4 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-slate-100 dark:border-slate-700/80">
                            <div>
                                <p class="font-semibold text-slate-800 dark:text-slate-100">{{ $d->title }}</p>
                                @if($d->description)
                                    <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">{{ $d->description }}</p>
                                @endif
                                ...
                                @if($d->feedback)
                                    <p class="text-sm mt-2 p-2 bg-amber-50 dark:bg-amber-900/30 rounded-lg text-amber-800 border border-amber-100"><strong>{{ __('instructor.admin_notes_label') }}:</strong> {{ $d->feedback }}</p>
                                @endif
                            </div>
                            <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold
                                @if($d->status === 'approved') bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400
                                @elseif($d->status === 'rejected' || $d->status === 'needs_revision') bg-rose-100 dark:bg-rose-900/40 text-rose-700 dark:text-rose-400
            ...
            <form action="{{ route('instructor.tasks.submit-deliverable', $task) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('instructor.submission_title_label') }} <span class="text-rose-500">*</span></label>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('instructor.submission_type_label') }}</label>
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('instructor.description_optional') }}</label>
                    <textarea name="description" rows="2" class="w-full px-4 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-800 dark:text-slate-100 focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20" placeholder="{{ __('instructor.submission_description_placeholder') }}">{{ old('description') }}</textarea>
                </div>
                <div id="file_input" class="flex items-center gap-2">
                    <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300">{{ __('instructor.file_label') }}</label>
                    ...
                    <span class="text-xs text-slate-500 dark:text-slate-400">{{ __('instructor.max_10mb') }}</span>
                </div>
                <div id="link_input" class="hidden">
                    <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('instructor.submission_link_label') }}</label>
