...
<div class="space-y-6">
    <div class="rounded-2xl bg-white dark:bg-slate-800/95 border border-slate-200 dark:border-slate-700 shadow-sm p-5 sm:p-6">
        ...
        <h1 class="text-xl font-bold text-slate-800 dark:text-slate-100">{{ __('instructor.edit_task') }}</h1>
        <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">{{ __('instructor.update_status_or_details') }}</p>
    </div>
    ...
    <div class="rounded-2xl bg-white dark:bg-slate-800/95 border border-slate-200 dark:border-slate-700 shadow-sm p-5 sm:p-6">
        <form action="{{ route('instructor.tasks.update', $task) }}" method="POST" class="space-y-6">
            ...
            <div>
                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('instructor.task_title_required') }}</label>
            ...
            <div>
                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('instructor.description') }}</label>
                <textarea name="description" rows="4"
                          class="w-full px-4 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-800 dark:text-slate-100 focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20 transition-colors">{{ old('description', $task->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('instructor.priority') }}</label>
            ...
            <div>
                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('instructor.due_date') }}</label>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('instructor.course_optional') }}</label>
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('instructor.lecture_optional') }}</label>
            ...
            <div class="flex flex-wrap gap-3 pt-4 border-t border-slate-200 dark:border-slate-700">
                <a href="{{ route('instructor.tasks.show', $task) }}" class="inline-flex items-center gap-2 px-5 py-2.5 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800/95 text-slate-700 dark:text-slate-300 rounded-xl font-semibold hover:bg-slate-50 dark:bg-slate-800/40 transition-colors">
