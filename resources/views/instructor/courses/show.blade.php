...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6" x-data="{ activeTab: 'overview' }">
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div class="flex-1 min-w-0">
                <nav class="text-sm text-slate-500 dark:text-slate-400 mb-2">
                    ...
                    <span class="text-slate-700 dark:text-slate-300 font-semibold truncate block sm:inline">{{ $course->title }}</span>
                </nav>
                <h1 class="text-xl sm:text-2xl font-bold text-slate-800 dark:text-slate-100 mb-3">{{ $course->title }}</h1>
                <div class="flex flex-wrap items-center gap-2">
                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold {{ $course->is_active ? 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400' : 'bg-red-100 text-red-700' }}">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        <div class="stats-mini-card rounded-xl p-4 text-center">
            ...
            <div class="text-xl font-bold text-slate-800 dark:text-slate-100">{{ $stats['total_lectures'] }}</div>
            <div class="text-xs text-slate-600 dark:text-slate-400 font-medium mt-1">{{ __('instructor.lecture_single') }}</div>
        </div>
        <div class="stats-mini-card rounded-xl p-4 text-center">
            <div class="w-10 h-10 rounded-xl bg-violet-100 text-violet-600 flex items-center justify-center mx-auto mb-2">
                <i class="fas fa-clipboard-check text-sm"></i>
            </div>
            <div class="text-xl font-bold text-slate-800 dark:text-slate-100">{{ $stats['total_exams'] }}</div>
            <div class="text-xs text-slate-600 dark:text-slate-400 font-medium mt-1">{{ __('instructor.exam_single') }}</div>
        </div>
        <div class="stats-mini-card rounded-xl p-4 text-center">
            <div class="w-10 h-10 rounded-xl bg-amber-100 text-amber-600 flex items-center justify-center mx-auto mb-2">
                <i class="fas fa-tasks text-sm"></i>
            </div>
            <div class="text-xl font-bold text-slate-800 dark:text-slate-100">{{ $stats['total_assignments'] }}</div>
            <div class="text-xs text-slate-600 dark:text-slate-400 font-medium mt-1">{{ __('instructor.assignment_single') }}</div>
        </div>
        <div class="stats-mini-card rounded-xl p-4 text-center">
            ...
            <div class="text-xl font-bold text-slate-800 dark:text-slate-100">{{ $stats['total_students'] }}</div>
            <div class="text-xs text-slate-600 dark:text-slate-400 font-medium mt-1">{{ __('instructor.student_single') }}</div>
        </div>
    </div>
            ...
            <div class="flex flex-wrap items-center gap-1 sm:gap-2 px-3 sm:px-4 py-2 overflow-x-auto">
                ...
                <button @click="activeTab = 'lectures'" 
                    ...
                    @if($stats['upcoming_lectures'] > 0)
                        <span class="absolute top-1 right-1 w-4 h-4 bg-red-600 dark:bg-red-700 text-white rounded-full text-[10px] flex items-center justify-center font-bold">{{ $stats['upcoming_lectures'] }}</span>
                    @endif
                </button>
                ...
                <button @click="activeTab = 'assignments'" 
                    ...
                    @if($stats['pending_submissions'] > 0)
                        <span class="absolute top-1 right-1 w-4 h-4 bg-red-600 dark:bg-red-700 text-white rounded-full text-[10px] flex items-center justify-center font-bold">{{ $stats['pending_submissions'] }}</span>
                    @endif
                </button>
        ...
        <div class="p-4 sm:p-6">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="content-card rounded-xl p-5 sm:p-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <div class="mb-3">
                                        <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1 uppercase tracking-wide">{{ __('instructor.title') }}</label>
                                        <div class="font-black text-slate-800 dark:text-slate-100 text-base">{{ $course->title }}</div>
                            </div>
                            @if($course->instructor)
                                    <div class="mb-3">
                                        <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1 uppercase tracking-wide">{{ __('instructor.instructor_label') }}</label>
                                        <div class="text-slate-800 dark:text-slate-100 font-bold">{{ $course->instructor->name }}</div>
                            </div>
                            @endif
                                    @if($course->level)
                                    <div class="mb-3">
                                        <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1 uppercase tracking-wide">{{ __('instructor.level') }}</label>
                                ...
                                <div>
                                    ...
                                    <div class="mb-3">
                                        <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1 uppercase tracking-wide">{{ __('instructor.course_duration') }}</label>
                                        <div class="text-slate-800 dark:text-slate-100 font-bold">
                            ...
                            @if($course->programming_language)
                                    <div class="mb-3">
                                        <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1 uppercase tracking-wide">{{ __('instructor.programming_language') }}</label>
                                        <div class="text-slate-800 dark:text-slate-100 font-bold">{{ $course->programming_language }}</div>
                            </div>
                            @endif
                        </div>
                    </div>
                    ...
                    @if($course->description)
                                <div class="mt-4 pt-4 border-t-2 border-slate-200 dark:border-slate-700">
                                    <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-2 uppercase tracking-wide">{{ __('instructor.description') }}</label>
                    ...
                    @if($course->objectives)
                                <div class="mt-4 pt-4 border-t-2 border-slate-200 dark:border-slate-700">
                                    <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-2 uppercase tracking-wide">{{ __('instructor.objectives') }}</label>
                                    <div class="text-slate-800 dark:text-slate-100 font-medium bg-slate-50 dark:bg-slate-800/40 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                                {{ $course->objectives }}
                            </div>
                        </div>
                            ...
                            <div class="space-y-2">
                                <a href="{{ route('instructor.lectures.create', ['course_id' => $course->id]) }}" 
                                   class="flex items-center gap-3 p-3 bg-sky-50 dark:bg-sky-900/30 hover:bg-sky-100 rounded-xl border border-sky-200 transition-all">
                                    ...
                                    <span class="font-bold text-slate-800 dark:text-slate-100 text-sm">{{ __('instructor.add_lecture') }}</span>
                                </a>
                                <a href="{{ route('instructor.exams.create', ['course_id' => $course->id]) }}" 
                                   class="flex items-center gap-3 p-3 bg-gradient-to-r from-indigo-500/10 to-purple-500/10 hover:from-indigo-500/20 hover:to-purple-500/20 rounded-xl border border-indigo-500/20 transition-all">
                                    ...
                                    <span class="font-bold text-slate-800 dark:text-slate-100 text-sm">{{ __('instructor.create_exam') }}</span>
                                </a>
                                <a href="{{ route('instructor.assignments.create', ['course_id' => $course->id]) }}" 
                                   class="flex items-center gap-3 p-3 bg-amber-50 dark:bg-amber-900/30 hover:bg-amber-100 rounded-xl border border-amber-200 transition-all">
                                    ...
                                    <span class="font-bold text-slate-800 dark:text-slate-100 text-sm">{{ __('instructor.create_assignment') }}</span>
                                </a>
                            </div>
                        </div>
                        ...
                        <div class="content-card rounded-xl p-5">
                            ...
                            <div class="space-y-3">
                                <div class="flex items-center justify-between p-2 bg-slate-50 dark:bg-slate-800/40 rounded-lg">
                                    <span class="text-xs text-slate-600 dark:text-slate-400 font-semibold">{{ __('instructor.upcoming_lectures') }}</span>
                                    <span class="font-black text-slate-800 dark:text-slate-100">{{ $stats['upcoming_lectures'] }}</span>
                                </div>
                                <div class="flex items-center justify-between p-2 bg-gradient-to-r from-indigo-500/5 to-purple-500/5 rounded-lg">
                                    <span class="text-xs text-slate-600 dark:text-slate-400 font-semibold">{{ __('instructor.active_exams') }}</span>
                                    <span class="font-black text-slate-800 dark:text-slate-100">{{ $stats['active_exams'] }}</span>
                                </div>
                                <div class="flex items-center justify-between p-2 bg-amber-50 dark:bg-amber-900/30 rounded-lg">
                                    <span class="text-xs text-slate-600 dark:text-slate-400 font-semibold">{{ __('instructor.pending_submissions') }}</span>
                                    <span class="font-black text-slate-800 dark:text-slate-100">{{ $stats['pending_submissions'] }}</span>
                                </div>
                                <div class="flex items-center justify-between p-2 bg-gradient-to-r from-blue-500/5 to-cyan-500/5 rounded-lg">
                                    <span class="text-xs text-slate-600 dark:text-slate-400 font-semibold">{{ __('instructor.attendance_records') }}</span>
                                    <span class="font-black text-slate-800 dark:text-slate-100">{{ $stats['total_attendance_records'] }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div x-show="activeTab === 'lectures'" x-transition style="display: none;">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="item-row flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-slate-200 dark:border-slate-700">
                            <div class="flex items-center gap-4 flex-1">
                                ...
                                <div class="flex-1">
                                    <div class="font-black text-slate-800 dark:text-slate-100 mb-1">{{ $lecture->title }}</div>
                                ...
                                <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold shadow-md
                                    ...
                                    @elseif($lecture->status == 'in_progress') bg-amber-500 dark:bg-amber-600 text-white
                ...
                @else
                    <div class="text-center py-12">
                        ...
                        <p class="text-lg font-black text-slate-800 dark:text-slate-100 mb-2">{{ __('instructor.no_lectures') }}</p>
                        <a href="{{ route('instructor.lectures.create', ['course_id' => $course->id]) }}" 
            ...
            <div x-show="activeTab === 'exams'" x-transition style="display: none;">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="item-row flex items-center justify-between p-4 bg-gradient-to-r from-indigo-500/5 to-purple-500/5 rounded-xl border border-indigo-500/10">
                            <div class="flex items-center gap-4 flex-1">
                                ...
                                <div class="flex-1">
                                    <div class="font-black text-slate-800 dark:text-slate-100 mb-1">{{ $exam->title }}</div>
                                    <div class="text-sm text-slate-600 dark:text-slate-400 font-medium">
                                ...
                                <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold shadow-md {{ $exam->is_active ? 'bg-gradient-to-r from-green-500 to-emerald-600 text-white' : 'bg-amber-500 dark:bg-amber-600 text-white' }}">
                ...
                @else
                    <div class="text-center py-12">
                        ...
                        <p class="text-lg font-black text-slate-800 dark:text-slate-100 mb-2">{{ __('instructor.no_exams') }}</p>
                        <a href="{{ route('instructor.exams.create') }}" 
            ...
            <div x-show="activeTab === 'assignments'" x-transition style="display: none;">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="item-row flex items-center justify-between p-4 bg-amber-50 dark:bg-amber-900/30 rounded-xl border border-amber-100">
                            <div class="flex items-center gap-4 flex-1">
                                ...
                                <div class="flex-1">
                                    <div class="font-black text-slate-800 dark:text-slate-100 mb-1">{{ $assignment->title }}</div>
                                    <div class="text-sm text-slate-600 dark:text-slate-400 font-medium">
                                ...
                                <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold shadow-md
                                    ...
                                    @elseif($assignment->status == 'draft') bg-amber-500 dark:bg-amber-600 text-white
                ...
                @else
                    <div class="text-center py-12">
                        ...
                        <p class="text-lg font-black text-slate-800 dark:text-slate-100 mb-2">{{ __('instructor.no_assignments') }}</p>
                        <a href="{{ route('instructor.assignments.create') }}" 
                ...
                <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-sky-50 dark:bg-sky-900/30">
                                <tr>
                                    <th class="px-4 py-3 text-right text-xs font-bold text-slate-800 dark:text-slate-100 uppercase tracking-wider">{{ __('instructor.name') }}</th>
                                    <th class="px-4 py-3 text-right text-xs font-bold text-slate-800 dark:text-slate-100 uppercase tracking-wider">{{ __('instructor.email') }}</th>
                                    <th class="px-4 py-3 text-right text-xs font-bold text-slate-800 dark:text-slate-100 uppercase tracking-wider">{{ __('instructor.phone') }}</th>
                                    <th class="px-4 py-3 text-right text-xs font-bold text-slate-800 dark:text-slate-100 uppercase tracking-wider">{{ __('instructor.registration_date') }}</th>
                                    <th class="px-4 py-3 text-right text-xs font-bold text-slate-800 dark:text-slate-100 uppercase tracking-wider">{{ __('common.status') }}</th>
                                    <th class="px-4 py-3 text-right text-xs font-bold text-slate-800 dark:text-slate-100 uppercase tracking-wider">{{ __('instructor.actions') }}</th>
                            </tr>
                        </thead>
            ...
            @else
                <div class="text-center py-12">
                        ...
                        <p class="text-lg font-black text-slate-800 dark:text-slate-100 mb-2">{{ __('instructor.no_enrolled_students') }}</p>
                        <p class="text-sm text-slate-600 dark:text-slate-400 font-medium">{{ __('instructor.no_enrolled_description') }}</p>
                    </div>
                @endif
            </div>
            ...
            <div x-show="activeTab === 'attendance'" x-transition style="display: none;">
                ...
                @else
                    <div class="text-center py-12">
                        ...
                        <p class="text-lg font-black text-slate-800 dark:text-slate-100 mb-2">{{ __('instructor.no_attendance_records') }}</p>
                        <p class="text-sm text-slate-600 dark:text-slate-400 font-medium">{{ __('instructor.no_attendance_description') }}</p>
                </div>
            @endif
            </div>
        </div>
