            ...
            <div>
                ...
                @if($user->employeeJob)
                    <p class="text-sm text-gray-500 mt-3 flex flex-wrap items-center gap-2">
                        <span class="inline-flex items-center gap-1.5"><i class="fas fa-briefcase text-teal-600"></i>{{ $user->employeeJob->name }}</span>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($studentsPreview as $st)
                            @php $live = $liveByStudentId->get($st->id); @endphp
                            <tr class="hover:bg-teal-50/30">
                                <td class="px-4 py-3">
                                    <p class="font-semibold text-gray-900">{{ $st->name }}</p>
                                    <p class="text-xs text-gray-500">{{ $st->email }}</p>
                                </td>
                                <td class="px-4 py-3 whitespace-nowrap text-gray-700">
                                    {{ $st->last_login_at ? $st->last_login_at->diffForHumans() : '—' }}
                                </td>
                                <td class="px-4 py-3">
                                    @if($live)
                ...
                <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                    ...
                    <ul class="space-y-2 max-h-64 overflow-y-auto">
                        @forelse($tasks as $task)
                            <li>
                                <a href="{{ route('employee.tasks.show', $task) }}" class="block rounded-lg border border-gray-100 px-3 py-2 hover:border-teal-200 hover:bg-teal-50/40 transition-colors">
                                    <p class="text-xs font-bold text-gray-900 line-clamp-1">{{ $task->title }}</p>
                                    <p class="text-[10px] text-gray-500 mt-0.5">{{ $task->status }}</p>
                                </a>
                            </li>
                        @empty
