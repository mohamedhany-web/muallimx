...
<body>
    <div class="box">
        ...
        <p>
            مرحباً {{ $student?->name ?? 'طالبنا العزيز' }}،
        </p>
        <p>
        ...
        <div class="card">
            ...
            <div class="value">{{ $course?->title ?? 'الكورس' }}</div>
        </div>

        @if($course && $course->academicYear)
            <div class="card">
                <div class="label">المسار / السنة الأكاديمية</div>
                <div class="value">{{ $course->academicYear->name }}</div>
            </div>
        @endif
        ...
        <a href="{{ url(route('public.course.show', $course?->id ?? 0)) }}" class="btn">
