...
@section('title', 'التحقق من الشهادة - ' . config('app.name'))
@section('meta_description', 'تحقق من صحة شهادات ' . config('app.name') . ' عبر رمز التحقق أو الرقم التسلسلي.')
@section('meta_keywords', 'تحقق شهادة, ' . config('app.name') . ', شهادات')
...
<section class="hero-gradient min-h-[32vh] flex items-center relative overflow-hidden pt-28 pb-10" style="background: linear-gradient(135deg, rgba(15, 23, 42, 0.9) 0%, rgba(30, 41, 59, 0.85) 25%, rgba(14, 165, 233, 0.7) 50%, rgba(14, 165, 233, 0.75) 75%, rgba(2, 132, 199, 0.8) 100%);">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
        ...
        <p class="text-white/95 text-sm md:text-base max-w-xl mx-auto">
            أدخل رمز التحقق أو السيريال للتحقق من صحة الشهادة الصادرة عن {{ config('app.name') }}
        </p>
    </div>
</section>
...
<div class="py-10 md:py-14 px-4">
    <div class="max-w-4xl mx-auto">
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-6 md:p-8 mb-8 border border-slate-100 dark:border-slate-700">
            <form method="GET" action="{{ route('public.certificates.verify') }}" class="flex flex-col sm:flex-row gap-4">
                <input type="text"
                       name="code"
                       value="{{ request('code') }}"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                            <div>
                                ...
                                <div class="space-y-2 text-sm">
                                    <div><span class="text-gray-600 dark:text-slate-400">الاسم:</span> <span class="font-semibold text-gray-900 dark:text-white">{{ $certificate->user->name ?? 'غير معروف' }}</span></div>
                                    <div><span class="text-gray-600 dark:text-slate-400">البريد:</span> <span class="font-semibold text-gray-900 dark:text-white">{{ $certificate->user->email ?? '-' }}</span></div>
                                </div>
                            </div>
                            <div>
                                <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4">معلومات الشهادة</h3>
                                <div class="space-y-2 text-sm">
                                    <div><span class="text-gray-600 dark:text-slate-400">رقم الشهادة:</span> <span class="font-semibold text-gray-900 dark:text-white font-mono">{{ $certificate->certificate_number }}</span></div>
                                    @if($certificate->serial_number)
                                    <div><span class="text-gray-600 dark:text-slate-400">السيريال:</span> <span class="font-semibold text-gray-900 dark:text-white font-mono">{{ $certificate->serial_number }}</span></div>
                                    @endif
                                    <div><span class="text-gray-600 dark:text-slate-400">تاريخ الإصدار:</span> <span class="font-semibold text-gray-900 dark:text-white">{{ $certificate->issued_at ? $certificate->issued_at->format('Y-m-d') : '-' }}</span></div>
            ...
            @else
                <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-8 border-2 border-red-500">
                    <div class="text-center">
                        ...
                        <p class="text-red-600 dark:text-red-400 font-semibold">{{ $error ?? 'تم اكتشاف تلاعب في الشهادة أو الشهادة غير موجودة' }}</p>
                    </div>
                </div>
            @endif
        @elseif(isset($error))
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-8 border-2 border-amber-500">
                <div class="text-center">
                    ...
                    <p class="text-amber-700 dark:text-amber-300 font-semibold">{{ $error }}</p>
                </div>
            </div>
        @endif
    </div>
