...
<head>
    ...
    <title>فصل {{ $teacher->name }} — Muallimx Classroom</title>
...
<body class="bg-slate-950 text-white min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md rounded-2xl bg-slate-800/90 border border-slate-600 p-6 shadow-2xl shadow-black/30 text-center">
        ...
        <p class="text-slate-300 text-sm mb-4">فصل: <span class="font-bold">{{ $teacher->name }}</span></p>
        ...
        @elseif($activeMeeting)
            <p class="text-emerald-300 text-sm font-semibold mb-2">الجلسة مباشرة الآن</p>
            @if($activeMeeting->title)
                <p class="text-slate-400 text-sm mb-4">{{ $activeMeeting->title }}</p>
            @endif
            <a href="{{ url('classroom/join/'.$activeMeeting->code) }}"
            ...
            <script>setTimeout(function(){ window.location.href = @json(url('classroom/join/'.$activeMeeting->code)); }, 800);</script>
        @else
            <div id="wait-box">
        ...
        <p class="text-slate-600 text-[11px] mt-6 break-all" dir="ltr">{{ $fixedUrl }}</p>
    </div>

    @if($serviceAvailable && ! $activeMeeting)
    <script>
