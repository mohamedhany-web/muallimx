    ...
    <div class="flex min-h-screen">
        ...
        <aside class="flex flex-col w-64 fixed inset-y-0 z-50 bg-slate-900 border-{{ $rtl ? 'l' : 'r' }} border-slate-700/50 transition-transform duration-300 ease-out {{ $rtl ? 'right-0' : 'left-0' }}"
            ...
            <nav class="flex-1 p-3 space-y-0.5 overflow-y-auto">
                ...
                <a href="{{ route('community.contributor.dashboard') }}" @click="if(!isLg) sidebarOpen = false" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-cyan-300 hover:bg-slate-800 hover:text-white {{ request()->routeIs('community.contributor.dashboard') ? 'bg-cyan-600/30 text-white' : '' }}">
                    <i class="fas fa-user-edit w-5"></i>
                    <span>لوحة المساهم</span>
                </a>
                <a href="{{ route('community.contributor.profile.edit') }}" @click="if(!isLg) sidebarOpen = false" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-cyan-300 hover:bg-slate-800 hover:text-white {{ request()->routeIs('community.contributor.profile.*') ? 'bg-cyan-600/30 text-white' : '' }}">
