...
<div class="w-full max-w-md">
    <div class="bg-slate-800/90 backdrop-blur border border-slate-700 rounded-2xl shadow-2xl p-6 sm:p-8">
        <h1 class="text-2xl font-black text-white mb-1">{{ __('auth.register') }}</h1>
        <p class="text-slate-400 text-sm mb-6">{{ __('auth.register_subtitle') }}</p>
        ...
        <form method="POST" action="{{ route('community.register.post') }}" class="space-y-4">
            @csrf
            <div>
                <label for="name" class="block text-sm font-semibold text-slate-300 mb-1">{{ __('auth.full_name') }}</label>
            ...
            <div>
                <label class="block text-sm font-semibold text-slate-300 mb-1">{{ __('auth.phone_number') }}</label>
            ...
            <div>
                <label for="email" class="block text-sm font-semibold text-slate-300 mb-1">{{ __('auth.email_optional') }}</label>
            ...
            <div>
                <label for="password_confirmation" class="block text-sm font-semibold text-slate-300 mb-1">{{ __('auth.password_confirmation') }}</label>
            ...
            <div class="flex items-start gap-2">
                ...
                <label for="terms" class="text-xs text-slate-400">{{ __('auth.agree_terms') }} <a href="{{ url('/terms') }}" class="text-cyan-400 hover:underline">{{ __('auth.terms_of_use') }}</a> {{ __('auth.and') }} <a href="{{ url('/privacy') }}" class="text-cyan-400 hover:underline">{{ __('auth.privacy_policy') }}</a></label>
            </div>
        ...
        <p class="mt-6 text-center text-slate-400 text-sm">
            ...
            <a href="{{ route('community.login') }}" class="text-cyan-400 font-bold hover:text-cyan-300">{{ __('auth.go_to_login') }}</a>
        </p>
        <p class="mt-2 text-center text-slate-500 text-xs">
            <a href="{{ route('register') }}" class="hover:text-slate-400">إنشاء حساب من المنصة الرئيسية</a>
        </p>
    </div>
</div>
@endsection
