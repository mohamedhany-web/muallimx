...
<div class="w-full">
    ...
    <form action="{{ route('community.contributor.models.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 bg-white rounded-2xl border border-slate-200 shadow-sm p-6" id="modelForm">
        @csrf
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3" placeholder="وصف مختصر للنموذج، الاستخدامات، الإطار المستخدم..."
                      class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 resize-y">{{ old('description') }}</textarea>
        </div>
        ...
        <div>
            ...
            <textarea name="methodology_steps" id="methodology_steps" rows="10" required
                      ...
                      class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 resize-y">{{ old('methodology_steps') }}</textarea>
        ...
        <div>
            ...
            <select name="community_dataset_id" id="community_dataset_id" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20">
                ...
                @foreach($datasets as $ds)
                    <option value="{{ $ds->id }}" {{ (string)old('community_dataset_id') === (string)$ds->id ? 'selected' : '' }}>{{ $ds->title }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <textarea name="performance_metrics" id="performance_metrics" rows="4" placeholder='{"accuracy": 0.92, "f1": 0.89, "loss": 0.15}'
                      class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 font-mono text-sm resize-y">{{ old('performance_metrics') }}</textarea>
        ...
        <div>
            ...
            <select name="license" id="license" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20">
                ...
                @foreach(\App\Models\CommunityModel::LICENSES as $key => $label)
                    <option value="{{ $key }}" {{ old('license') === $key ? 'selected' : '' }}>{{ $label }}</option>
                @endforeach
            </select>
        </div>
        ...
        {{-- رفع ملفات النموذج — أوزان، سكربتات بايثون، أرشيفات — تخزين على Cloudflare R2 --}}
        <div>
            ...
            <div id="dropZone" class="relative border-2 border-dashed border-slate-300 rounded-2xl p-8 text-center bg-slate-50/50 hover:bg-amber-50/30 hover:border-amber-400 transition-colors cursor-pointer">
                ...
                <div class="pointer-events-none">
                    ...
                    <p class="text-slate-500 text-sm">أوزان/أخرى: pkl, pt, pth, h5, onnx, joblib, zip, safetensors, bin, json — حتى {{ (int)(\App\Http\Controllers\Community\ContributorController::MAX_MODEL_FILE_KB / 1024) }} ميجا لكل ملف، حد أقصى 10 ملفات</p>
                </div>
            </div>
        ...
        <div class="flex flex-wrap gap-3 pt-2">
            ...
            <a href="{{ route('community.contributor.models.index') }}" class="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-slate-100 text-slate-700 font-bold hover:bg-slate-200 transition-colors">إلغاء</a>
        </div>
    </form>
</div>
...
(function() {
    const MAX_FILES = 10;
    const MAX_MB = {{ (int)(\App\Http\Controllers\Community\ContributorController::MAX_MODEL_FILE_KB / 1024) }};
