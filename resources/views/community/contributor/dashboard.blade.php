...
<div class="w-full space-y-6">
    ...
    @endif
    @if(session('warning'))
        <div class="p-4 rounded-xl bg-amber-100 border border-amber-300 text-amber-800">{{ session('warning') }}</div>
    @endif
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <a href="{{ route('community.contributor.datasets.index') }}" class="group block bg-white rounded-2xl border border-slate-200 p-5 shadow-sm hover:shadow-lg hover:border-cyan-200 transition-all duration-300">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-slate-500 text-sm font-semibold mb-1">إجمالي التقديمات</p>
                    <p class="text-3xl font-black text-slate-800">{{ $myDatasetsCount ?? 0 }}</p>
                </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <p class="text-3xl font-black text-amber-600">{{ $pendingCount ?? 0 }}</p>
                </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <p class="text-3xl font-black text-emerald-600">{{ $approvedCount ?? 0 }}</p>
                </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <p class="text-3xl font-black text-red-600">{{ $rejectedCount ?? 0 }}</p>
                </div>
    ...
    <div class="flex flex-wrap gap-3">
        ...
        <a href="{{ route('community.contributor.datasets.index') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-slate-100 text-slate-700 font-bold hover:bg-slate-200 transition-colors">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/70 flex items-center justify-between">
            ...
            <a href="{{ route('community.contributor.datasets.index') }}" class="text-sm font-bold text-cyan-600 hover:text-cyan-700">عرض الكل</a>
        </div>
        <div class="divide-y divide-slate-100">
            @foreach($recentSubmissions as $d)
            <div class="px-6 py-4 flex flex-wrap items-center justify-between gap-3">
                <div>
                    <p class="font-bold text-slate-900">{{ $d->title }}</p>
                    <p class="text-sm text-slate-500">{{ $d->created_at->diffForHumans() }}</p>
                </div>
                <div class="flex items-center gap-2 flex-wrap">
                    <span class="inline-flex px-3 py-1 rounded-full text-xs font-bold
                        @if($d->status === 'pending') bg-amber-100 text-amber-700
                        @elseif($d->status === 'approved') bg-emerald-100 text-emerald-700
                        @else bg-red-100 text-red-700 @endif">
                        @if($d->status === 'pending') قيد المراجعة
                        @elseif($d->status === 'approved') معتمدة
                        @else مرفوضة @endif
                    </span>
                    @if($d->status === 'approved' && $d->is_active)
                        @php $shareUrl = 'https://www.linkedin.com/sharing/share-offsite/?url=' . urlencode(url(route('community.data.show', $d))); @endphp
                        <a href="{{ $shareUrl }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-[#0A66C2] text-white text-xs font-bold hover:bg-[#004182] transition-colors" title="مشاركة على LinkedIn — ساهمت في مجتمع البيانات والذكاء الاصطناعي">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 p-8 text-center">
        ...
        <a href="{{ route('community.contributor.datasets.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-cyan-600 text-white font-bold hover:bg-cyan-700 transition-colors">
            <i class="fas fa-plus"></i>
