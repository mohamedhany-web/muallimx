...
<div class="w-full space-y-8">
    ...
    <div class="relative rounded-3xl overflow-hidden bg-gradient-to-br from-cyan-600 via-blue-600 to-slate-800 p-8 sm:p-10 shadow-2xl text-white">
        ...
        <div class="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
            <div>
                ...
                <h1 class="text-2xl sm:text-3xl lg:text-4xl font-black mb-2 tracking-tight">مرحباً، {{ $user->name }}!</h1>
            ...
            <div class="flex flex-wrap gap-3 shrink-0">
                <a href="{{ route('community.data.index') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white/15 hover:bg-white/25 backdrop-blur border border-white/20 font-bold text-sm transition-all">
                    <i class="fas fa-database"></i>
                    <span>صفحة البيانات</span>
                </a>
                <a href="{{ route('community.models.index') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white/15 hover:bg-white/25 backdrop-blur border border-white/20 font-bold text-sm transition-all">
    ...
    {{-- إحصائيات — 4 كروت بتصميم منسق --}}
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <a href="{{ route('community.competitions.index') }}" class="group block bg-white rounded-2xl border border-slate-200/80 p-6 shadow-sm hover:shadow-xl hover:border-amber-200 transition-all duration-300">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-slate-500 text-sm font-bold mb-1">المسابقات</p>
                    <p class="text-3xl font-black text-amber-600">{{ $stats['competitions_count'] ?? 0 }}</p>
        ...
        <a href="{{ route('community.data.index') }}" class="group block bg-white rounded-2xl border border-slate-200/80 p-6 shadow-sm hover:shadow-xl hover:border-blue-200 transition-all duration-300">
            <div class="flex items-start justify-between">
                <div>
        ...
        <a href="{{ route('community.models.index') }}" class="group block bg-white rounded-2xl border border-slate-200/80 p-6 shadow-sm hover:shadow-xl hover:border-amber-200 transition-all duration-300">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-slate-500 text-sm font-bold mb-1">مكتبة النماذج</p>
                    <p class="text-3xl font-black text-amber-600">{{ $stats['models_count'] ?? 0 }}</p>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8">
        ...
        <div class="flex flex-wrap gap-3">
            <a href="{{ route('community.competitions.index') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-amber-500 text-white font-bold hover:bg-amber-600 transition-colors shadow-md hover:shadow-lg">
                <i class="fas fa-trophy"></i>
                <span>المسابقات</span>
            </a>
            <a href="{{ route('community.data.index') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-blue-600 text-white font-bold hover:bg-blue-700 transition-colors shadow-md hover:shadow-lg">
                <i class="fas fa-database"></i>
                <span>مجموعات البيانات</span>
            </a>
            <a href="{{ route('community.models.index') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-amber-600 text-white font-bold hover:bg-amber-700 transition-colors shadow-md hover:shadow-lg">
                <i class="fas fa-brain"></i>
                <span>مكتبة النماذج</span>
            </a>
    ...
    {{-- ثلاثة أعمدة: مسابقات، بيانات، نماذج --}}
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="p-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
                ...
                <a href="{{ route('community.competitions.index') }}" class="text-sm font-bold text-amber-600 hover:underline">عرض الكل</a>
            </div>
            <div class="p-4">
                    ...
                    <ul class="space-y-2">
                        @foreach($recentCompetitions as $c)
                            <li class="flex items-center gap-3 p-3 rounded-xl bg-slate-50/80 hover:bg-amber-50/80 border border-transparent hover:border-amber-100 transition-colors">
                                ...
                                <a href="{{ route('community.competitions.index') }}" class="text-amber-600 hover:text-amber-700 text-xs font-semibold flex-shrink-0">عرض</a>
                            </li>
                        @endforeach
                    </ul>
                @else
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="p-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
                ...
                <a href="{{ route('community.data.index') }}" class="text-sm font-bold text-blue-600 hover:underline">عرض الكل</a>
            </div>
            <div class="p-4">
                    ...
                    <ul class="space-y-2">
                        @foreach($recentDatasets as $d)
                            <li class="flex items-center gap-3 p-3 rounded-xl bg-slate-50/80 hover:bg-blue-50/80 border border-transparent hover:border-blue-100 transition-colors">
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="font-bold text-slate-900 truncate text-sm">{{ $d->title }}</p>
                                    @if($d->file_size)
                                        <p class="text-xs text-slate-500">{{ $d->file_size }}</p>
                                    @endif
                                </div>
                                <a href="{{ route('community.data.show', $d) }}" class="text-blue-600 hover:text-blue-700 text-xs font-semibold flex-shrink-0">عرض</a>
                            </li>
                        @endforeach
                    </ul>
                @else
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="p-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
                ...
                <a href="{{ route('community.models.index') }}" class="text-sm font-bold text-amber-600 hover:underline">عرض الكل</a>
            </div>
            <div class="p-4">
                @if(isset($recentModels) && $recentModels->isNotEmpty())
                    <ul class="space-y-2">
                        @foreach($recentModels as $m)
                            <li class="flex items-center gap-3 p-3 rounded-xl bg-slate-50/80 hover:bg-amber-50/80 border border-transparent hover:border-amber-100 transition-colors">
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="font-bold text-slate-900 truncate text-sm">{{ $m->title }}</p>
                                    @if($m->license)
                                        <p class="text-xs text-slate-500">{{ Str::limit($m->license, 20) }}</p>
                                    @endif
                                </div>
                                <a href="{{ route('community.models.show', $m) }}" class="text-amber-600 hover:text-amber-700 text-xs font-semibold flex-shrink-0">عرض</a>
                            </li>
                        @endforeach
                    </ul>
                @else
    ...
    <div class="rounded-2xl border border-slate-200 bg-gradient-to-r from-slate-50 to-white p-6 text-center">
        <p class="text-slate-600 text-sm">هل لديك اقتراح أو سؤال؟ تواصل معنا من <a href="{{ route('public.contact') }}" class="text-cyan-600 font-bold hover:underline">صفحة التواصل</a>.</p>
    </div>
</div>
@endsection
