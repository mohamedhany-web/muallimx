    ...
    <div class="container-1200 relative z-10">
      ...
      <div class="grid grid-cols-2 gap-3 sm:gap-4 mt-10 max-w-xl mx-auto reveal s2">
        <article class="rounded-2xl p-4 sm:p-5 border border-slate-200 bg-white text-center shadow-[0_10px_24px_-18px_rgba(31,42,122,.25)]">
          ...
          <p class="text-xs sm:text-sm text-slate-600 mt-1">{{ __('public.courses_stats_available') }}</p>
        </article>
        <article class="rounded-2xl p-4 sm:p-5 border border-slate-200 bg-[#f8faff] text-center shadow-[0_10px_24px_-18px_rgba(31,42,122,.25)]">
          ...
          <p class="text-xs sm:text-sm text-slate-700 mt-1">{{ __('public.courses_stats_featured') }}</p>
        </article>
      </div>
    </div>
  </section>
                ...
                <div>
                  ...
                  <template x-if="!course.is_free && course.price > 0 && course.has_promo_price && course.sale_price < course.price">
                    <div class="flex flex-col gap-0.5">
                      <span class="text-xs text-slate-400 line-through tabular-nums"><span x-text="course.price"></span> {{ __('public.currency_egp') }}</span>
                      <div><span class="text-xl font-black text-mx-orange tabular-nums" x-text="course.sale_price"></span> <span class="text-xs text-slate-400">{{ __('public.currency_egp') }}</span></div>
                    </div>
                  </template>
                  <template x-if="!course.is_free && course.price > 0 && (!course.has_promo_price || course.sale_price >= course.price)">
                    <div><span class="text-xl font-black text-mx-orange tabular-nums" x-text="course.sale_price != null ? course.sale_price : course.price"></span> <span class="text-xs text-slate-400">{{ __('public.currency_egp') }}</span></div>
                  </template>
                </div>
