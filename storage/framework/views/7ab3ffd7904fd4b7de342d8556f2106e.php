                        ...
                        <div class="grid sm:grid-cols-2 gap-3">
                            ...
                            <div>
                                ...
                                <select x-model.number="form.weekday" @change="rebuildMonthDays()" class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm">
                                    <?php $__currentLoopData = $weekdayLabels; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($i); ?>"><?php echo e($label); ?></option>
...
function teacherPersonalCalendar() {
    ...
    const weekdayLabels = <?php echo json_encode($weekdayLabels, 15, 512) ?>;
    const initialTzOptions = <?php echo json_encode(collect($timezoneOptions ?? [])->map(fn ($label, $id) => ['id' => $id, 'label' => $label])->values()) ?>;
    const usStatesMap = <?php echo json_encode($usStates ?? [], 15, 512) ?>;
    ...
    return {
        ...
        form: {
            ...
            teacher_timezone: <?php echo json_encode($teacherTimezone, 15, 512) ?>,
            family_time: '17:00',
            duration_minutes: 60,
            weekday: 6,
        ...
        async searchTimezones(q) {
            try {
                const url = <?php echo json_encode(route('calendar.personal.timezones'), 15, 512) ?> + (q ? ('?q=' + encodeURIComponent(q)) : '');
        ...
        async applyUsState() {
            ...
            try {
                const res = await fetch(<?php echo json_encode(route('calendar.personal.us-state'), 15, 512) ?>, {
                    method: 'POST',
        ...
        async refreshPreview() {
            ...
            try {
                const res = await fetch(<?php echo json_encode(route('calendar.personal.preview'), 15, 512) ?>, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrf, 'Accept': 'application/json' },
                    body: JSON.stringify({
                        date,
        ...
        async submitForm() {
            ...
            this.errorText = '';
            try {
                const res = await fetch(<?php echo json_encode(route('calendar.personal.store'), 15, 512) ?>, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrf, 'Accept': 'application/json' },
...
<?php $__env->stopPush(); ?>
<?php /**PATH C:\xampp\htdocs\Muallimx\resources\views/student/calendar/_personal-appointment-modal.blade.php ENDPATH**/ ?>