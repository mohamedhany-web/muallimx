...
<?php $__env->startSection('title', 'بنك الأسئلة: ' . $questionBank->title . ' - ' . config('app.name', 'Muallimx')); ?>
...
<div class="w-full px-4 sm:px-6 lg:px-8 py-6" x-data="{ showCreateModal: false }">
    ...
    <div class="rounded-2xl bg-white dark:bg-slate-800/95 border border-slate-200 dark:border-slate-700 shadow-sm p-5 sm:p-6 mb-6">
        <nav class="text-sm text-slate-500 dark:text-slate-400 mb-2">
            ...
            <span class="text-slate-700 dark:text-slate-300 font-semibold"><?php echo e($questionBank->title); ?></span>
        </nav>
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div class="flex flex-wrap items-center gap-4">
                ...
                <div class="min-w-0">
                    <h1 class="text-xl sm:text-2xl font-bold text-slate-800 dark:text-slate-100"><?php echo e($questionBank->title); ?></h1>
                    <p class="text-sm text-slate-600 dark:text-slate-400 mt-0.5"><?php echo e($questionBank->description ?? 'بنك أسئلة'); ?></p>
                </div>
            </div>
    ...
    <div class="rounded-2xl bg-white dark:bg-slate-800/95 border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 flex items-center justify-between flex-wrap gap-2">
            <h2 class="text-lg font-bold text-slate-800 dark:text-slate-100">الأسئلة (<?php echo e($questionBank->questions->count()); ?>)</h2>
            <button type="button" @click="showCreateModal = true"
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="flex gap-3 mb-2">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <p class="font-semibold text-slate-800 dark:text-slate-100"><?php echo e(Str::limit($question->question, 200)); ?></p>
                                ...
                                <?php if($question->type == 'multiple_choice' && $question->options && is_array($question->options)): ?>
                                    <div class="mt-2 flex flex-wrap gap-2">
                                        <?php $__currentLoopData = $question->options; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $opt): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <span class="px-2 py-1 rounded-lg text-sm bg-slate-100 dark:bg-slate-700/50 text-slate-700 dark:text-slate-300 <?php echo e((!is_array($question->correct_answer) && $opt == $question->correct_answer) || (is_array($question->correct_answer) && in_array($opt, $question->correct_answer)) ? 'ring-2 ring-green-500 bg-green-50 text-green-800' : ''); ?>">
                                ...
                                <?php elseif(in_array($question->type, ['true_false', 'short_answer', 'essay'])): ?>
                                    <p class="mt-2 text-sm text-slate-600 dark:text-slate-400">الإجابة: <?php echo e(is_array($question->correct_answer) ? ($question->correct_answer[0] ?? '—') : $question->correct_answer); ?></p>
                                <?php endif; ?>
                            </div>
                            <div class="flex items-center gap-2 shrink-0">
                                <a href="<?php echo e(route('instructor.questions.edit', $question)); ?>" class="p-2 rounded-lg text-sky-600 hover:bg-sky-50 dark:bg-sky-900/30 transition-colors" title="تعديل">
                                    <i class="fas fa-edit"></i>
                                </a>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\Muallimx\resources\views\instructor\question-banks\show.blade.php ENDPATH**/ ?>