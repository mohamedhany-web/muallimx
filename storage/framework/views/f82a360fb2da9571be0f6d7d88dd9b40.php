    ...
    <?php if($datasets->isNotEmpty()): ?>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <?php $__currentLoopData = $datasets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dataset): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <a href="<?php echo e(route('community.datasets.show', $dataset)); ?>" class="block bg-white rounded-2xl border border-slate-200 p-6 shadow-sm hover:shadow-md transition-shadow">
                    <div class="w-12 h-12 rounded-xl bg-blue-100 text-blue-600 flex items-center justify-center mb-4">
                        <i class="fas fa-database text-xl"></i>
                    </div>
                    <h3 class="text-lg font-black text-slate-900 mb-2"><?php echo e($dataset->title); ?></h3>
                    <?php if($dataset->description): ?>
                        <p class="text-slate-600 text-sm mb-4 line-clamp-3"><?php echo e(Str::limit($dataset->description, 120)); ?></p>
                    <?php endif; ?>
                    <div class="flex flex-wrap items-center gap-2">
                        <?php if($dataset->file_size): ?>
                            <span class="text-xs text-slate-500"><?php echo e($dataset->file_size); ?></span>
                        <?php endif; ?>
        ...
        <?php if($datasets->hasPages()): ?>
            <div class="mt-8"><?php echo e($datasets->links()); ?></div>
        <?php endif; ?>
    <?php else: ?>
...
<?php echo $__env->make('community.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\mindly tics\Mindlytics\resources\views/community/datasets/index.blade.php ENDPATH**/ ?>