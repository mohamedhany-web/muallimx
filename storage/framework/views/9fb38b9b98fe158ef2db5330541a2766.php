        ...
        <div class="learn-focus-content lg:col-span-8 xl:col-span-9">
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden min-h-[400px]">
                <div class="focus-main-content-wrapper p-4 lg:p-6">
                    ...
                    <div x-show="(selectedLecture && !showVideoPlayer) || selectedExam" x-transition class="lesson-content-viewer">
...
function courseFocusMode() {
    ...
    return {
        ...
        loadExam(examId) {
            ...
            const examShowUrl = '<?php echo e(route("student.exams.show", ":ID")); ?>'.replace(':ID', examId);

            // عرض صفحة الامتحان كاملة داخل كادر في نفس منطقة التعلم (بدون مغادرة الصفحة)
            this.lectureContent =
                '<div class="exam-embed-wrapper w-full h-full min-h-[420px] rounded-2xl border border-slate-200 overflow-hidden bg-slate-50">' +
                    '<iframe src="' + examShowUrl + '?embed=1"' +
                            ' class="w-full h-[520px] md:h-[620px] lg:h-[700px]"' +
                            ' frameborder="0"' +
                            ' allowfullscreen>' +
                    '</iframe>' +
                '</div>';
