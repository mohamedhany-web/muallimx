        ...
        <div class="container-1200 relative z-10">
            <div class="max-w-4xl mx-auto text-center reveal">
                ...
                <div class="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
                    <a class="btn-primary inline-flex items-center justify-center gap-2 !bg-[#FB5607] hover:!bg-[#e84d00]" href="<?php echo e(route('register')); ?>">ابدأ مجاناً <i class="fas fa-user-plus text-xs"></i></a>
                    <a class="btn-secondary inline-flex items-center justify-center gap-2 !bg-[#283593] !text-white !border-[#283593] hover:!bg-[#1f2a7a]" href="<?php echo e(route('public.pricing')); ?>#plans">عرض الباقات <i class="fas fa-tags text-xs"></i></a>
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mt-10 reveal s2">
                ...
                <article class="rounded-2xl p-4 sm:p-5 border border-slate-200 bg-white text-center shadow-[0_10px_24px_-18px_rgba(31,42,122,.25)]">
                    <p class="text-3xl sm:text-4xl font-black text-[#FB5607]" dir="ltr"><?php echo e($fmt((int) ($hs['features'] ?? 0))); ?>+</p>
                ...
                <article class="rounded-2xl p-4 sm:p-5 border border-slate-200 bg-[#FFE5F7] text-center shadow-[0_10px_24px_-18px_rgba(31,42,122,.25)]">
                    <p class="text-3xl sm:text-4xl font-black text-mx-indigo" dir="ltr"><?php echo e($fmt((int) ($hs['plans'] ?? 3))); ?></p>
                ...
                <?php $__currentLoopData = $offers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $offer): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <article class="card-base hover-lift h-full">
                        <div class="w-11 h-11 rounded-xl mb-3 flex items-center justify-center text-[#FB5607]" style="background:#fff3ec">
                            <i class="fas <?php echo e($offer['icon']); ?>"></i>
                        </div>
                        <h3 class="font-heading font-bold text-mx-indigo mb-2"><?php echo e($offer['title']); ?></h3>
                        <p class="text-sm text-slate-600 leading-7"><?php echo e($offer['desc']); ?></p>
                    </article>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
            <p class="text-center text-sm text-slate-500 mt-8 reveal">
                تريد التفاصيل والأسعار؟
                <a href="<?php echo e(route('public.pricing')); ?>#plans" class="font-bold text-[#FB5607] underline underline-offset-2">اطلع على الباقات من هنا</a>
            </p>
        </div>
    </section>
    ...
    <section class="py-12 sm:py-16 bg-white">
        <div class="container-1200">
            ...
            <div class="grid lg:grid-cols-3 gap-4 relative max-w-5xl mx-auto">
                ...
                <?php $steps=[['أنشئ حسابك','تسجيل سريع ثم تدخل لوحة المعلم مباشرة.','fa-user-plus'],['اختر باقتك','مجانية للتجربة أو أساسية/شاملة حسب احتياجك.','fa-tags'],['فعّل أدواتك','مناهج، AI، فيديو، كلاس رووم — من داخل حسابك.','fa-rocket']]; ?>
    ...
    <section id="home-plans" class="py-14 sm:py-16 bg-mx-soft scroll-mt-24">
        <div class="container-1200">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 max-w-6xl mx-auto">
                <?php $__currentLoopData = $planKeys; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $planKey): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    ...
                    <article class="reveal card-base hover-lift !p-7 flex flex-col relative
                        <?php echo e($isPro ? 'border-[#283593] ring-2 ring-[#283593]/10' : ($isFree ? 'border-emerald-400 ring-2 ring-emerald-500/15' : 'border-slate-200')); ?>">
                        <?php if($badge !== ''): ?>
                            <div class="absolute top-3 left-4 bg-[#FB5607] text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg z-10"><?php echo e($badge); ?></div>
                        <?php endif; ?>
                        <h3 class="text-2xl font-black text-mx-indigo mb-1"><?php echo e($label); ?></h3>
                        <?php if($subtitle !== ''): ?>
                            <p class="text-sm font-semibold text-[#283593] mb-4"><?php echo e($subtitle); ?></p>
                        <?php endif; ?>
                        <div class="mb-5">
                            <?php if($isFree): ?>
                                <p class="text-3xl font-black text-mx-indigo">مجاناً</p>
                                <p class="text-sm font-bold text-emerald-700 mt-1">لمدة <?php echo e((int) ($plan['duration_days'] ?? 14)); ?> يوماً</p>
                            <?php else: ?>
                                <p class="text-3xl font-black text-mx-indigo"><?php echo e(number_format($price, 0)); ?> <span class="text-base font-bold text-slate-600"><?php echo e($cyclePhrase); ?></span></p>
                            <?php endif; ?>
                        </div>
                        <ul class="space-y-2.5 text-sm text-slate-700 mb-7 flex-1">
                            <?php $__currentLoopData = $features; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $featureKey): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <li class="flex items-start gap-2">
                                    ...
                                    <span class="font-semibold"><?php echo e(__("student.subscription_feature.{$featureKey}")); ?></span>
                                </li>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </ul>
                        <?php if($isFree): ?>
                            <?php if(auth()->guard()->check()): ?>
                                <form action="<?php echo e(route('public.subscription.activate-free')); ?>" method="POST" class="w-full">
                                    ...
                                    <button type="submit" class="w-full inline-flex items-center justify-center px-6 py-3 rounded-xl font-bold text-sm bg-emerald-600 hover:bg-emerald-700 text-white"><?php echo e($cta); ?></button>
                                </form>
                            <?php else: ?>
                                <a href="<?php echo e(route('login', ['intended' => route('public.pricing')])); ?>" class="w-full inline-flex items-center justify-center px-6 py-3 rounded-xl font-bold text-sm bg-emerald-600 hover:bg-emerald-700 text-white">سجّل الدخول للتفعيل المجاني</a>
                            <?php endif; ?>
                        <?php else: ?>
                            <a href="<?php echo e(route('public.subscription.checkout', $planKey)); ?>" class="w-full inline-flex items-center justify-center px-6 py-3 rounded-xl font-bold text-sm <?php echo e($isPro ? 'bg-[#283593] hover:bg-[#1f2a7a] text-white' : 'bg-[#FB5607] hover:bg-[#e84d00] text-white'); ?>"><?php echo e($cta); ?></a>
                        <?php endif; ?>
                    </article>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
            ...
            <div class="text-center mt-8">
                <a href="<?php echo e(route('public.pricing')); ?>#plans" class="btn-secondary inline-flex items-center gap-2">تفاصيل أكثر عن الباقات <i class="fas fa-arrow-<?php echo e($isRtl ? 'left' : 'right'); ?> text-xs"></i></a>
            </div>
        </div>
    </section>
        ...
        <div class="container-1200">
            <div class="reveal rounded-[28px] border border-slate-200 bg-white shadow-[0_20px_44px_-26px_rgba(31,42,122,.28)] px-6 sm:px-10 py-10 sm:py-12 text-center">
                ...
                <div class="flex flex-col sm:flex-row justify-center gap-3 sm:gap-4">
                    ...
                    <a href="<?php echo e(route('public.pricing')); ?>#plans" class="btn-secondary inline-flex items-center justify-center gap-2">عرض الباقات</a>
                </div>
            </div>
        </div>
    </section>
