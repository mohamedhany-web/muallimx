<?php $__env->startSection('title', $label . ' — ' . __('student.full_ai_suite.layer1_title')); ?>
...
<div class="w-full min-w-0 min-h-[calc(100dvh-7rem)] sm:min-h-[calc(100dvh-8rem)] -mx-4 px-4 md:-mx-6 md:px-6 lg:-mx-8 lg:px-8 pb-8 space-y-6 sm:space-y-8">
    ...
    <div class="flex flex-wrap items-center justify-center gap-2 sm:gap-4 text-[11px] sm:text-xs font-semibold text-slate-500 dark:text-slate-400">
        <span class="hidden sm:inline text-slate-400 dark:text-slate-500"><?php echo e(__('student.full_ai_suite.step_intro')); ?></span>
        <div class="flex items-center gap-2 sm:gap-3">
            ...
            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-sky-100 dark:bg-sky-900/40 text-sky-800 dark:text-sky-200 border border-sky-200/80 dark:border-sky-800/60">
                ...
                <?php echo e(__('student.full_ai_suite.step_context_label')); ?>
    ...
    <div class="w-full rounded-3xl bg-white dark:bg-slate-800/95 border border-slate-200/90 dark:border-slate-700 shadow-xl shadow-slate-200/40 dark:shadow-none overflow-hidden ring-1 ring-slate-100/80 dark:ring-slate-700/50">
        <div class="relative overflow-hidden bg-gradient-to-br from-violet-50 via-white to-fuchsia-50/40 dark:from-violet-950/50 dark:via-slate-800 dark:to-slate-900 p-6 sm:p-8 lg:p-10 border-b border-slate-100 dark:border-slate-700/80">
            ...
            <div class="relative grid grid-cols-1 xl:grid-cols-[auto_minmax(0,1fr)] gap-6 xl:gap-10 xl:items-start">
                <div class="flex shrink-0 items-center justify-center w-16 h-16 sm:w-[4.5rem] sm:h-[4.5rem] xl:w-20 xl:h-20 rounded-2xl <?php echo e($iconBg); ?> <?php echo e($iconText); ?> shadow-inner ring-4 ring-white/80 dark:ring-slate-800/80 mx-auto xl:mx-0">
                    <i class="fas <?php echo e($icon); ?> text-2xl sm:text-[1.65rem] xl:text-3xl"></i>
                </div>
                <div class="flex-1 min-w-0 space-y-3 text-center xl:text-start">
                    <div class="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:gap-2 xl:justify-start justify-center">
                        <h1 class="text-2xl sm:text-3xl xl:text-4xl font-black tracking-tight text-slate-900 dark:text-white"><?php echo e($label); ?></h1>
                        <span class="inline-flex w-fit mx-auto sm:mx-0 items-center gap-2 px-3 py-1.5 rounded-xl bg-emerald-500/10 dark:bg-emerald-400/10 text-emerald-800 dark:text-emerald-200 text-xs font-bold border border-emerald-300/40 dark:border-emerald-600/40 shadow-sm">
                            ...
                            <?php echo e(__('student.full_ai_suite.layer1_title')); ?>

                        </span>
                    </div>
                    <p class="text-sm sm:text-[15px] text-slate-600 dark:text-slate-400 leading-relaxed max-w-none"><?php echo e($description); ?></p>
        ...
        <div class="p-5 sm:p-8 lg:p-10">
            <div class="flex items-start gap-3 mb-6 lg:mb-8">
                ...
                <div class="min-w-0">
                    <h2 class="text-lg lg:text-xl font-black text-slate-900 dark:text-white"><?php echo e(__('student.full_ai_suite.layer2_title')); ?></h2>
                    <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1 leading-relaxed max-w-none"><?php echo e(__('student.full_ai_suite.layer2_hint')); ?></p>
                </div>
            </div>
            ...
            <?php if($errors->any()): ?>
                <div class="mb-6 rounded-2xl bg-red-50 dark:bg-red-950/30 border border-red-200/90 dark:border-red-800/60 px-4 py-3.5 text-sm text-red-800 dark:text-red-200 space-y-1.5 shadow-sm">
                    <?php $__currentLoopData = $errors->all(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $err): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <p class="flex items-start gap-2"><i class="fas fa-circle-exclamation mt-0.5 shrink-0 opacity-80"></i><span><?php echo e($err); ?></span></p>
            ...
            <?php if($courses->isEmpty()): ?>
                <div class="rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-600 bg-gradient-to-b from-slate-50/90 to-white dark:from-slate-900/50 dark:to-slate-800/30 px-6 py-12 sm:py-16 text-center">
                    ...
                    <p class="text-sm text-slate-600 dark:text-slate-400 max-w-2xl mx-auto leading-relaxed"><?php echo e(__('student.full_ai_suite.course_empty')); ?></p>
                    <a href="<?php echo e(route('public.courses')); ?>" class="mt-5 inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-sky-600 hover:bg-sky-700 text-white text-sm font-bold shadow-md shadow-sky-600/20 transition-colors">
            ...
            <?php else: ?>
                <form action="<?php echo e(route('student.features.full-ai-suite.preview')); ?>" method="POST" class="space-y-6 w-full">
                    <?php echo csrf_field(); ?>
                    <div class="rounded-2xl border border-slate-200/90 dark:border-slate-600/80 bg-slate-50/70 dark:bg-slate-900/40 p-5 sm:p-6 lg:p-8 space-y-5 lg:space-y-6 shadow-inner w-full">
                        ...
                        <div class="space-y-2 w-full" x-data="{ len: <?php echo e(strlen(old('question', ''))); ?> }">
                            ...
                            <textarea name="question" id="question" rows="7" required minlength="10" maxlength="4000"
                                ...
                                class="w-full min-h-[160px] lg:min-h-[180px] rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 px-4 py-3 text-sm leading-relaxed shadow-sm focus:ring-2 focus:ring-violet-500/80 focus:border-violet-500 transition resize-y"><?php echo e(old('question')); ?></textarea>
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-center gap-3 lg:gap-6 w-full">
                        ...
                        <p class="text-[11px] text-slate-500 dark:text-slate-500 text-center lg:text-start flex-1 leading-relaxed"><?php echo e(__('student.full_ai_suite.preview_hint_short')); ?></p>
                    </div>
                </form>
            <?php endif; ?>
        </div>
            ...
            <div class="px-5 sm:px-8 lg:px-10 py-5 sm:py-6 border-b border-slate-100 dark:border-slate-700/80 bg-gradient-to-l from-slate-50 to-white dark:from-slate-800/80 dark:to-slate-900/90">
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                        <h2 class="text-lg lg:text-xl font-black text-slate-900 dark:text-white"><?php echo e(__('student.full_ai_suite.layer3_title')); ?></h2>
                        <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1 leading-relaxed"><?php echo e(__('student.full_ai_suite.layer3_note')); ?></p>
                    </div>
                </div>
                
                <div class="xl:hidden mt-4 flex rounded-xl bg-slate-100/90 dark:bg-slate-900/60 p-1 gap-1">
                    <button type="button" @click="tab = 'json'" :class="tab === 'json' ? 'bg-white dark:bg-slate-800 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500'" class="flex-1 rounded-lg px-3 py-2 text-xs font-bold transition"><?php echo e(__('student.full_ai_suite.context_json_title')); ?></button>
                    <button type="button" @click="tab = 'prompt'" :class="tab === 'prompt' ? 'bg-white dark:bg-slate-800 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500'" class="flex-1 rounded-lg px-3 py-2 text-xs font-bold transition"><?php echo e(__('student.full_ai_suite.prompt_preview_title')); ?></button>
                </div>
            </div>
            ...
            <div class="p-5 sm:p-8 lg:p-10">
                ...
                <div class="xl:hidden space-y-6">
                    <div x-show="tab === 'json'" x-cloak class="space-y-3" x-data="{ copied: false }">
                        <div class="flex items-center justify-between gap-2">
                            <span class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide"><?php echo e(__('student.full_ai_suite.context_json_title')); ?></span>
                        ...
                        <pre x-ref="jsonBlockSm" class="text-[11px] sm:text-xs leading-relaxed p-4 sm:p-5 rounded-2xl bg-[#0d1117] text-[#7ee787] overflow-x-auto border border-slate-700/80 font-mono shadow-inner max-h-[min(50vh,480px)] min-h-[200px] overflow-y-auto" dir="ltr"><?php echo e($jsonPreview); ?></pre>
                    </div>
                    <div x-show="tab === 'prompt'" x-cloak class="space-y-3" x-data="{ copied: false }">
                        <div class="flex items-center justify-between gap-2">
                            <span class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide"><?php echo e(__('student.full_ai_suite.prompt_preview_title')); ?></span>
                        ...
                        <pre x-ref="promptBlockSm" class="text-[11px] sm:text-xs leading-relaxed p-4 sm:p-5 rounded-2xl bg-slate-100 dark:bg-slate-950 text-slate-800 dark:text-slate-200 overflow-x-auto border border-slate-200 dark:border-slate-700 whitespace-pre-wrap font-mono shadow-inner max-h-[min(50vh,480px)] min-h-[200px] overflow-y-auto" dir="ltr"><?php echo e($promptText); ?></pre>
                    </div>
                </div>
                ...
                <div class="hidden xl:grid xl:grid-cols-2 xl:gap-8 w-full min-w-0">
                    <div class="space-y-3 min-w-0 flex flex-col" x-data="{ copied: false }">
                        ...
                        <pre x-ref="jsonBlockLg" class="flex-1 text-[11px] sm:text-xs leading-relaxed p-4 sm:p-5 rounded-2xl bg-[#0d1117] text-[#7ee787] overflow-x-auto border border-slate-700/80 font-mono shadow-inner min-h-[280px] max-h-[min(60vh,560px)] overflow-y-auto w-full" dir="ltr"><?php echo e($jsonPreview); ?></pre>
                    </div>
                    <div class="space-y-3 min-w-0 flex flex-col" x-data="{ copied: false }">
                        <div class="flex items-center justify-between gap-2">
                            <span class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide"><?php echo e(__('student.full_ai_suite.prompt_preview_title')); ?></span>
                        ...
                        <pre x-ref="promptBlockLg" class="flex-1 text-[11px] sm:text-xs leading-relaxed p-4 sm:p-5 rounded-2xl bg-slate-100 dark:bg-slate-950 text-slate-800 dark:text-slate-200 overflow-x-auto border border-slate-200 dark:border-slate-700 whitespace-pre-wrap font-mono shadow-inner min-h-[280px] max-h-[min(60vh,560px)] overflow-y-auto w-full" dir="ltr"><?php echo e($promptText); ?></pre>
                    </div>
                </div>
            </div>
        </div>
