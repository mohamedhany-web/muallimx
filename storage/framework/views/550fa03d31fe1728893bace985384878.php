    ...
    <?php else: ?>
        <div class="bg-white dark:bg-slate-800 rounded-xl p-5 sm:p-6 border border-gray-200 dark:border-slate-700 shadow-sm space-y-5">
            ...
            <form action="<?php echo e(route('student.features.full-ai-suite.preview')); ?>" method="POST" class="space-y-5">
                <?php echo csrf_field(); ?>
                <div class="space-y-5" x-data="{ questionType: <?php echo json_encode(in_array(old('question_type'), array_keys($qTypes), true) ? old('question_type') : 'educational_tips') ?>, len: <?php echo e(strlen(old('question', ''))); ?> }">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <?php $__currentLoopData = $qTypes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $transKey): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <label class="relative flex items-start gap-3 p-4 rounded-xl border-2 cursor-pointer transition-colors"
                                       :class="questionType === '<?php echo e($key); ?>' ? 'border-sky-500 bg-sky-50 dark:bg-sky-950/30' : 'border-gray-200 dark:border-slate-600 hover:border-sky-300 dark:hover:border-sky-700'">
                                    <input type="radio" name="question_type" value="<?php echo e($key); ?>" class="mt-1 text-sky-600"
                                           x-model="questionType" <?php if(old('question_type', 'educational_tips') === $key): echo 'checked'; endif; ?> required>
                                    <span class="min-w-0">
                                        <span class="block text-sm font-bold text-gray-900 dark:text-slate-100"><?php echo e(__($transKey)); ?></span>
                                        <span class="block text-xs text-gray-500 dark:text-slate-400 mt-1 leading-relaxed">
                                            <?php if($key === 'educational_games'): ?>
                                                <?php echo e(__('student.full_ai_suite.question_type_game_hint')); ?>

                                            <?php else: ?>
                                                <?php echo e(__('student.full_ai_suite.question_type_tips_hint')); ?>

                                            <?php endif; ?>
                                        </span>
                                    </span>
                                    <i class="fas <?php echo e($key === 'educational_games' ? 'fa-gamepad' : 'fa-lightbulb'); ?> text-lg ms-auto shrink-0"
                                       :class="questionType === '<?php echo e($key); ?>' ? 'text-sky-600' : 'text-gray-400'"></i>
                                </label>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </div>
                    </div>
                ...
                <div class="rounded-xl border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/25 p-4 text-sm text-red-800 dark:text-red-200">
                    ...
                    <?php if($gameHtmlUrl !== ''): ?>
                        <p class="mt-3 text-xs text-amber-800 dark:text-amber-200 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-lg px-3 py-2">
                            <?php echo e(__('student.full_ai_suite.game_busy_static_fallback_notice')); ?>
