    ...
    <div class="space-y-6 max-w-4xl">
        ...
        <div class="flex flex-wrap items-start justify-between gap-4">
            ...
            <div class="flex flex-wrap items-center gap-2">
                <form method="POST"
                      action="<?php echo e(route('admin.n8n.live-session-reports.regenerate', ['source' => $source, 'report' => $reportView->id])); ?>"
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6 shadow-sm">
            ...
            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                <div>
                    ...
                    <dd class="font-medium text-slate-800 dark:text-white"><?php echo e($reportView->user?->name ?? '—'); ?>

                        <?php if($reportView->user_id): ?><span class="text-slate-400 text-xs">(#<?php echo e($reportView->user_id); ?>)</span><?php endif; ?>
                    </dd>
                </div>
                <div>
                    <dt class="text-slate-500 dark:text-slate-400"><?php echo e($reportView->source === 'live_session' ? 'الجلسة' : 'الاجتماع'); ?></dt>
                    <dd class="font-medium text-slate-800 dark:text-white"><?php echo e($reportView->context_title ?? '—'); ?>
            ...
            <div class="p-6">
                <?php if($reportView->summary): ?>
                    <div id="report-summary-body"
                         class="text-sm text-slate-800 dark:text-slate-100 whitespace-pre-wrap leading-relaxed rounded-lg border border-slate-100 dark:border-slate-600 bg-slate-50/80 dark:bg-slate-900/50 px-4 py-4 max-h-[70vh] overflow-y-auto"><?php echo e($reportView->summary); ?></div>
