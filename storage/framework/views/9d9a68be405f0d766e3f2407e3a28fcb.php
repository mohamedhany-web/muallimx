<?php $__env->startSection('title', 'نظام الاتفاقيات - ' . config('app.name', 'Muallimx')); ?>
            ...
            <form method="GET" action="<?php echo e(route('admin.agreements.index')); ?>" id="filterForm" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="search" value="<?php echo e(htmlspecialchars(request('search') ?? '', ENT_QUOTES, 'UTF-8')); ?>" maxlength="255" placeholder="رقم الاتفاقية، اسم المدرب" class="w-full rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-4 py-2.5 pr-10 text-sm text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all" />
                    </div>
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="type" class="w-full rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-4 py-2.5 text-sm text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all">
                        ...
                        <option value="consultation_session" <?php echo e(request('type') == 'consultation_session' ? 'selected' : ''); ?>>استشارات</option>
                    </select>
                </div>
                <div>
                ...
                <div class="flex items-end gap-2 md:col-span-4">
                    ...
                    <a href="<?php echo e(route('admin.agreements.index')); ?>" class="inline-flex items-center justify-center rounded-xl border border-slate-300 dark:border-slate-600 px-4 py-2.5 text-sm font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors" title="مسح الفلتر">
                ...
                <tbody class="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-700">
                    <?php $__empty_1 = true; $__currentLoopData = $agreements; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $agreement): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/40 transition-colors duration-200">
                            <td class="px-6 py-4">
                                <div class="font-semibold text-slate-900 dark:text-slate-100"><?php echo e(htmlspecialchars($agreement->agreement_number ?? 'N/A', ENT_QUOTES, 'UTF-8')); ?></div>
                                <div class="text-xs text-slate-500 dark:text-slate-400 mt-1"><?php echo e(htmlspecialchars($agreement->title ?? '', ENT_QUOTES, 'UTF-8')); ?></div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="font-semibold text-slate-900 dark:text-slate-100"><?php echo e(htmlspecialchars($agreement->instructor->name ?? 'غير محدد', ENT_QUOTES, 'UTF-8')); ?></p>
                                        <p class="text-xs text-slate-500 dark:text-slate-400"><?php echo e(htmlspecialchars($agreement->instructor->phone ?? '-', ENT_QUOTES, 'UTF-8')); ?></p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <div class="font-bold text-slate-900 dark:text-slate-100"><?php echo e(number_format($agreement->rate ?? 0, 2)); ?> ج.م</div>
                            ...
                            <td class="px-6 py-4 text-xs text-slate-700 dark:text-slate-300">
                                ...
                                <?php if($agreement->end_date): ?>
                                    <div class="text-slate-500 dark:text-slate-400">حتى <?php echo e($agreement->end_date->format('Y-m-d')); ?></div>
                                <?php endif; ?>
                            </td>
                            <td class="px-6 py-4">
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\Muallimx\resources\views\admin\agreements\index.blade.php ENDPATH**/ ?>