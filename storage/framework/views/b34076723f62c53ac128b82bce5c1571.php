...
<div class="max-w-6xl mx-auto space-y-6 pb-10">
    ...
    <?php if(session('success')): ?>
        <div class="rounded-2xl border border-emerald-200 bg-emerald-50 dark:bg-emerald-900/20 dark:border-emerald-800 px-5 py-4 flex items-start gap-3 shadow-sm">
            ...
            <p class="text-emerald-900 dark:text-emerald-100 text-sm font-semibold leading-relaxed pt-1.5"><?php echo e(session('success')); ?></p>
        </div>
    <?php endif; ?>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
        ...
        <div class="lg:col-span-7 space-y-4">
            ...
            <?php else: ?>
                <div class="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm overflow-hidden">
                    <div class="px-4 py-3 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between gap-2 bg-slate-50/80 dark:bg-slate-900/40">
                        <span class="text-xs font-bold text-slate-600 dark:text-slate-300">النتائج (<?php echo e($students->count()); ?>)</span>
                            ...
                            <li>
                                <label class="flex items-center gap-4 px-4 py-3.5 cursor-pointer hover:bg-sky-50/60 dark:hover:bg-slate-700/40 transition-colors has-[:checked]:bg-sky-50 dark:has-[:checked]:bg-sky-900/20 has-[:checked]:ring-2 has-[:checked]:ring-inset has-[:checked]:ring-sky-400/50">
                                    ...
                                    <div class="min-w-0 flex-1 text-start">
                                        <p class="font-bold text-slate-900 dark:text-white text-sm truncate"><?php echo e($s->name); ?></p>
            ...
            <?php if($selectedStudent && ($students->isEmpty() || ! $students->contains('id', $selectedStudent->id))): ?>
                <div class="rounded-2xl border border-violet-200 bg-violet-50 dark:bg-violet-900/20 dark:border-violet-800 px-4 py-3 text-sm">
                    ...
                    <p class="text-violet-900 dark:text-violet-100 font-semibold"><?php echo e($selectedStudent->name); ?></p>
                    <p class="text-violet-700/90 dark:text-violet-300/90 text-xs mt-1">معرف #<?php echo e($selectedStudent->id); ?> — أعد البحث ليظهر في القائمة إن أردت تغييره.</p>
                </div>
            <?php endif; ?>
        </div>
        ...
        <div class="lg:col-span-5">
            <form id="wallet-credit-store-form"
                ...
                <?php if($selectedStudent && ($students->isEmpty() || ! $students->contains('id', $selectedStudent->id))): ?>
                    <input type="hidden" name="user_id" value="<?php echo e($selectedStudent->id); ?>">
                <?php endif; ?>
...
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
                            <p class="text-red-600 dark:text-red-400 text-xs mt-1.5 font-medium"><?php echo e($message); ?></p>
                        <?php unset($message);
if (isset($__messageOriginal)) { $message = $__messageOriginal; }
endif;
unset($__errorArgs, $__bag); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\Muallimx\resources\views/admin/marketing/student-wallet-credit.blade.php ENDPATH**/ ?>