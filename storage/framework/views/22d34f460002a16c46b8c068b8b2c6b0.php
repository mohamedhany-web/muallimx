...
?>
    ...
    <header class="h-[72px] bg-gradient-to-l from-slate-900 to-slate-800 border-b border-slate-700/50 flex items-center justify-between px-4 sm:px-6 shadow-lg">
        ...
        <div class="flex items-center gap-2">
            ...
            <label class="inline-flex items-center gap-2 px-2.5 py-2 rounded-xl bg-slate-700/50 border border-slate-600 cursor-pointer select-none text-slate-200 text-xs sm:text-sm shrink-0"
                ...
                <input type="checkbox" id="mx-classroom-toggle-guest-wb" class="rounded border-slate-500 text-amber-500 focus:ring-amber-500 shrink-0"
                       <?php echo e($meeting->allowsParticipantWhiteboard() ? 'checked' : ''); ?>>
                <span class="font-medium whitespace-nowrap"><span class="hidden sm:inline">رسم الضيف فوق العرض</span><span class="sm:hidden">رسم ضيف</span></span>
            </label>
            <div class="relative shrink-0 inline-flex" id="mx-record-dd-wrap">
                <div id="mx-record-idle-wrap" class="inline-flex items-center rounded-xl border border-slate-600 overflow-hidden bg-slate-700/80 hover:bg-slate-600/90 transition-colors">
                    <button type="button" id="btn-record-menu" class="inline-flex items-center gap-2 px-3 sm:px-4 py-2 text-slate-200 text-sm font-medium" title="تسجيل المحاضرة أو تقرير صوتي" aria-expanded="false" aria-haspopup="true">
                        <i class="fas fa-circle-dot text-rose-400" id="record-icon-idle"></i>
                        <span id="record-label-idle">تسجيل أو تقرير</span>
                        <i class="fas fa-chevron-down text-[10px] text-slate-400" id="record-dd-chevron" aria-hidden="true"></i>
                    </button>
                </div>
                <button type="button" id="btn-record-stop" class="hidden inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-rose-600/90 hover:bg-rose-600 text-white text-sm font-semibold transition-colors border border-rose-500/50 shadow-lg shadow-rose-900/20" title="إيقاف التسجيل">
                    <i class="fas fa-stop" id="record-icon-active"></i>
                    <span id="record-label-active">إيقاف التسجيل</span>
                </button>
                <div id="mx-record-dd-panel" class="hidden absolute top-[calc(100%+0.5rem)] end-0 w-[min(100vw-2rem,20rem)] rounded-xl border border-slate-600 bg-slate-900/98 backdrop-blur-md overflow-hidden z-[160] shadow-xl" role="menu">
                    <p class="px-3 py-2 text-[11px] text-slate-500 border-b border-slate-700/80 m-0">يُرفع كل شيء إلى Cloudflare R2. التسجيل صوت الميكروفون فقط (دون مشاركة الشاشة).</p>
                    <button type="button" role="menuitem" data-mx-rec-mode="lecture" class="w-full text-right px-3 py-2.5 text-sm text-slate-200 hover:bg-slate-700/80 border-0 border-b border-slate-700/50 bg-transparent cursor-pointer flex items-center gap-2">
                        <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-rose-500/15 text-rose-300"><i class="fas fa-microphone"></i></span>
                        <span class="min-w-0 flex-1 leading-snug"><strong class="block text-slate-100">تسجيل المحاضرة</strong><span class="text-[11px] text-slate-500">صوت الميكروفون — يُحفظ كتسجيل المحاضرة</span></span>
                    </button>
                    <button type="button" role="menuitem" data-mx-rec-mode="report" class="w-full text-right px-3 py-2.5 text-sm text-slate-200 hover:bg-slate-700/80 border-0 bg-transparent cursor-pointer flex items-center gap-2">
                        <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-cyan-500/15 text-cyan-300"><i class="fas fa-file-audio"></i></span>
                        <span class="min-w-0 flex-1 leading-snug"><strong class="block text-slate-100">إنشاء تقرير</strong><span class="text-[11px] text-slate-500">صوت فقط في الخلفية — ملف التقرير الصوتي</span></span>
                    </button>
                </div>
            </div>
    ...
    <div id="mx-upload-modal" class="hidden fixed inset-0 z-[180] flex items-center justify-center p-4 bg-slate-950/75 backdrop-blur-sm" aria-hidden="true">
        <div class="w-full max-w-md rounded-2xl border border-slate-600 bg-slate-900 shadow-2xl p-5 sm:p-6" role="dialog" aria-labelledby="mx-upload-modal-title" aria-modal="true">
            ...
            <p id="mx-upload-modal-sub" class="text-xs text-slate-500 m-0 mb-4">الرفع مباشرة إلى Cloudflare R2. يمكنك تصغير هذه النافذة والمتابعة في الاجتماع.</p>
    ...
    <script>
        (function() {
            ...
            function putBlobToPresignedUrl(url, blob, contentType, extraHeaders, onPercent) {
                return new Promise(function(resolve, reject) {
                    ...
                    xhr.onload = function() {
                        ...
                        reject(new Error('فشل الرفع إلى Cloudflare R2 (HTTP ' + xhr.status + '). تحقق من CORS للدلوكل (PUT و Origin).'));
                    };
                    xhr.onerror = function() {
                        reject(new Error('انقطع الاتصال أثناء الرفع إلى R2.'));
                    };
                    xhr.ontimeout = function() {
                        reject(new Error('انتهت مهلة الرفع إلى R2.'));
                    };
                    xhr.send(blob);
                });
            }
            ...
            async function uploadRecordedBlob(blob, durationSeconds, onProgress) {
                ...
                try {
                    if (typeof onProgress === 'function') {
                        onProgress({ text: 'جاري تجهيز رابط الرفع إلى Cloudflare R2...', percent: 2, toChip: true });
                    }
                    ...
                    if (presignRes.ok && presignData.upload_url && presignData.upload_token && presignData.content_type) {
                        if (typeof onProgress === 'function') {
                            onProgress({ text: 'جاري الرفع إلى R2 (' + formatBytes(blob.size) + ')...', percent: 5, toChip: true });
                        }
                        await putBlobToPresignedUrl(
                            ...
                            function(p) {
                                if (typeof onProgress === 'function') {
                                    ...
                                    onProgress({ text: 'جاري الرفع إلى Cloudflare R2...', percent: scaled, toChip: true });
                                }
                            }
                        );
                        putSucceeded = true;
            ...
            async function uploadAudioBlob(blob, durationSeconds, onProgress) {
                ...
                if (typeof onProgress === 'function') {
                    onProgress({ text: 'جاري تجهيز رابط رفع التقرير إلى R2...', percent: 2, toChip: true });
                }
                ...
                if (typeof onProgress === 'function') {
                    onProgress({ text: 'جاري رفع التقرير الصوتي إلى R2...', percent: 5, toChip: true });
                }
                await putBlobToPresignedUrl(
                    presignData.upload_url,
                    blob,
            ...
            async function mxRunUploadJob(job) {
                ...
                mxReportUploadProgress({
                    text: 'جاري حفظ نسخة احتياطية محلية ثم البدء بالرفع إلى R2...',
                    percent: 1,
                    toChip: true,
                });
                ...
                try {
                    ...
                    mxReportUploadProgress({ text: 'اكتمل الرفع إلى Cloudflare R2 بنجاح.', percent: 100, toChip: true });
            ...
            function mxQueueBlobUpload(blob, durationSeconds, kind) {
                var job = {
                ...
                mxRunUploadJob(job).catch(function() {});
            }

            async function startMicRecording(kind) {
                ...
                try {
                    ...
                    try {
                        var fallback = recorderOpts.mimeType ? { mimeType: recorderOpts.mimeType, audioBitsPerSecond: 96000 } : { audioBitsPerSecond: 96000 };
                ...
                mediaRecorder.addEventListener('stop', async function onRecorderStopped() {
                    ...
                    setRecordButtonBusy(false);
                    if (stoppedKind === 'report') {
                        setRecordStatus('تم إيقاف تسجيل التقرير. جاري فتح نافذة الرفع...', false);
                    } else {
                        setRecordStatus('تم إيقاف تسجيل المحاضرة. جاري فتح نافذة الرفع...', false);
                    }
                    mxQueueBlobUpload(blob, durationSeconds, stoppedKind || 'lecture');
                    recordedChunks = [];
                });
                ...
                if (kind === 'report') {
                    ...
                    setRecordStatus('جاري تسجيل المحاضرة (صوت الميكروفون)...', false);
                }
                setRecordButtonBusy(false);
            }
            ...
            function stopBrowserRecording() {
                ...
                setRecordButtonBusy(true);
                setRecordStatus('جاري إنهاء التسجيل ودمج المقاطع...', false);
                try {
            ...
            if (btnRecordMenu && recordDdPanel && recordDdWrap) {
                ...
                recordDdPanel.querySelectorAll('[data-mx-rec-mode]').forEach(function(el) {
                    el.addEventListener('click', function() {
                        ...
                        setRecordDdOpen(false);
                        if (mode === 'lecture' || mode === 'report') {
                            startMicRecording(mode);
                        }
                    });
                });
            ...
            if (mxUploadChip) {
                mxUploadChip.addEventListener('click', function() {
                    if (mxLastFailedJob && mxLastFailedJob.blob && (mxLastFailedJob.status === 'failed' || mxLastFailedJob.status === 'uploading')) {
                        mxShowUploadModal(true);
                        if (mxUploadModalTitle) mxUploadModalTitle.textContent = 'رفع معلّق إلى R2';
                        if (mxUploadModalStatus) {
                            mxUploadModalStatus.textContent = 'يوجد ملف محفوظ محلياً ولم يُرفع بالكامل. اضغط «إعادة المحاولة» لإكمال الرفع إلى Cloudflare.';
                        }
                        if (mxUploadModalRetry) mxUploadModalRetry.classList.remove('hidden');
                        return;
                    }
            ...
            if (!window.__mxClassroomOnlineHook) {
                ...
                window.addEventListener('online', function() {
                    if (mxLastFailedJob && mxLastFailedJob.blob && (mxLastFailedJob.status === 'failed' || mxLastFailedJob.status === 'uploading')) {
                        ...
                        mxShowUploadModal(true);
                        if (mxUploadModalStatus) {
                            mxUploadModalStatus.textContent = 'عاد الاتصال — جاري إعادة المحاولة تلقائياً...';
                        }
                        mxRunUploadJob(retryJob).catch(function() {});
                    }
                });
            ...
            mxIdbListMeetingJobs().then(function(list) {
                ...
                setRecordStatus('يوجد رفع سابق غير مكتمل — اضغط على شريط الرفع بالأسفل لإعادة المحاولة.', true);
            }).catch(function() {});
