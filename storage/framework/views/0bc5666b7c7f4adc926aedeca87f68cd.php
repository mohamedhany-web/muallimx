...
<?php $__env->startSection('header', 'المصروفات'); ?>
    ...
    <section class="rounded-3xl bg-white/95 backdrop-blur border border-slate-200 shadow-lg overflow-hidden">
        <div class="px-5 py-6 sm:px-8 border-b border-slate-200">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
                ...
                <a href="<?php echo e(route('admin.accounting.reports.export', array_merge(request()->query(), ['type' => 'expenses']))); ?>" class="inline-flex items-center gap-2 rounded-2xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-emerald-700">
                    <i class="fas fa-file-excel"></i> تصدير Excel
                </a>
            </div>
            <form method="GET" action="<?php echo e(route('admin.accounting.reports.expenses')); ?>" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
            ...
            <table class="min-w-full divide-y divide-slate-200 text-right text-sm">
                ...
                <tbody class="divide-y divide-slate-200">
                    ...
                    <tr class="hover:bg-slate-50">
                        <td class="px-4 py-3"><a href="<?php echo e(route('admin.expenses.show', $expense)); ?>" class="font-semibold text-sky-600 hover:text-sky-700"><?php echo e($expense->expense_number ?? '—'); ?></a></td>
                        <td class="px-4 py-3"><?php echo e($expense->title ?? '—'); ?></td>
                        <td class="px-4 py-3"><?php echo e(\App\Models\Expense::categoryLabel($expense->category ?? '')); ?></td>
                        <td class="px-4 py-3 font-semibold text-rose-600"><?php echo e(number_format($expense->amount, 2)); ?> ج.م</td>
                        <td class="px-4 py-3">
                            <?php if($expense->status == 'approved'): ?><span class="rounded-full px-2 py-0.5 text-xs font-semibold bg-emerald-100 text-emerald-700">معتمد</span>
                            <?php elseif($expense->status == 'pending'): ?><span class="rounded-full px-2 py-0.5 text-xs font-semibold bg-amber-100 text-amber-700">معلق</span>
                            <?php else: ?><span class="rounded-full px-2 py-0.5 text-xs font-semibold bg-rose-100 text-rose-700">مرفوض</span><?php endif; ?>
                        </td>
                        <td class="px-4 py-3"><?php echo e($expense->expense_date ? $expense->expense_date->format('Y-m-d') : '—'); ?></td>
                    </tr>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\Muallimx\resources\views\admin\accounting\reports-expenses.blade.php ENDPATH**/ ?>