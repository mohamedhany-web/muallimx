    ...
    <div class="rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm p-6 space-y-4">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div class="flex items-center gap-2">
                <?php if(!$meeting->consultation_request_id && !($useInstructorRoutes ?? false) && !$meeting->isLive() && !$meeting->ended_at): ?>
        ...
        <div class="rounded-xl border border-dashed border-slate-300 dark:border-slate-600 p-3 space-y-3">
            <?php if(!empty($fixedJoinUrl) && !($useInstructorRoutes ?? false)): ?>
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div class="text-xs text-slate-600 dark:text-slate-300">الرابط الثابت (يُفضَّل مشاركته دائماً):</div>
                <div class="flex items-center gap-2">
                    <input type="text" readonly value="<?php echo e($fixedJoinUrl); ?>" class="w-[340px] max-w-[60vw] px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-xs" dir="ltr">
                    <button type="button" onclick="navigator.clipboard.writeText(<?php echo json_encode($fixedJoinUrl, 15, 512) ?>)" class="px-3 py-2 rounded-lg bg-[#FB5607]/15 text-[#FB5607] text-xs font-semibold">نسخ</button>
                </div>
            </div>
            <?php endif; ?>
            <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                <div class="flex items-center gap-2">
                    <input type="text" readonly value="<?php echo e($joinUrl); ?>" class="w-[340px] max-w-[60vw] px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-xs" dir="ltr">
                    <button type="button" onclick="navigator.clipboard.writeText(<?php echo json_encode($joinUrl, 15, 512) ?>)" class="px-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-700 text-xs font-semibold">نسخ</button>
                </div>
            </div>
        </div>
