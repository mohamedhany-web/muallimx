    ...
    <div class="content-wrapper min-h-screen flex flex-col"
        ...
        <header class="top-navbar sticky top-0 z-40 flex items-center px-5 lg:px-8 gap-4">
            ...
            <div class="flex items-center gap-2">
                    ...
                    <button type="button"
                        ...
                        <?php if($adminUnreadCount > 0): ?>
                            <span class="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] bg-rose-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center ring-2 ring-white dark:ring-slate-800 px-1">
                                <?php echo e($adminUnreadCount > 9 ? '9+' : $adminUnreadCount); ?>
                    ...
                    <div x-show="openNotif" x-cloak
                        ...
                        <div class="px-4 py-3 border-b border-slate-100 flex items-center justify-between bg-slate-50/80">
                            <div>
                                ...
                                <p class="text-xs text-slate-500 mt-0.5">
                                    <?php echo e($adminUnreadCount > 0 ? "لديك {$adminUnreadCount} إشعار غير مقروء" : 'لا توجد إشعارات جديدة حالياً'); ?>

                                </p>
                            </div>
                            <a href="<?php echo e(route('admin.notifications.index')); ?>" class="text-xs font-semibold text-sky-600 hover:text-sky-700">
                        ...
                        <div class="max-h-[320px] overflow-y-auto">
                            <?php $__empty_1 = true; $__currentLoopData = $adminUnreadNotifications; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $notif): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <a href="<?php echo e(route('admin.notifications.show', $notif)); ?>"
                                   class="flex items-start gap-3 px-4 py-3 hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-b-0">
                                    <div class="mt-0.5">
                                        <span class="inline-flex items-center justify-center w-8 h-8 rounded-xl text-xs font-semibold
                                            <?php if($notif->priority === 'urgent'): ?> bg-rose-100 text-rose-600
                                            <?php elseif($notif->priority === 'high'): ?> bg-amber-100 text-amber-600
                                            <?php else: ?> bg-sky-100 text-sky-600 <?php endif; ?>">
                                            <i class="<?php echo e($notif->type_icon); ?>"></i>
                                        </span>
                                    </div>
                                    <div class="flex-1 min-w-0">
                ...
                <!-- Settings -->
                <a href="<?php echo e(route('settings')); ?>" class="hidden sm:flex w-10 h-10 rounded-xl bg-slate-50 hover:bg-slate-100 items-center justify-center text-slate-400 transition-all active:scale-95">
                ...
                <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                    <button @click.stop="open = !open" class="flex items-center gap-2.5 px-2.5 py-2 rounded-xl hover:bg-slate-50 transition-all active:scale-[0.98]" :aria-expanded="open">
                        <?php if(auth()->user()->profile_image): ?>
                            <img src="<?php echo e(asset('storage/' . auth()->user()->profile_image)); ?>" alt="<?php echo e(auth()->user()->name); ?>" class="w-9 h-9 rounded-xl object-cover ring-2 ring-slate-100" onerror="this.style.display='none'; this.nextElementSibling?.classList.remove('hidden');">
                        ...
                        <div class="hidden lg:block text-right">
                            <p class="text-[13px] font-semibold text-slate-700 max-w-[100px] truncate leading-tight"><?php echo e(auth()->user()->name); ?></p>
                    ...
                    <div x-show="open" x-cloak
                        ...
                        <div class="px-4 py-3 bg-slate-50/80 border-b border-slate-100">
                            ...
                            <p class="text-xs text-slate-400 truncate mt-0.5"><?php echo e(auth()->user()->email ?? auth()->user()->phone); ?></p>
                        </div>
                        <div class="py-1.5">
                            <a href="<?php echo e(route('admin.dashboard')); ?>" class="flex items-center gap-3 px-4 py-2.5 text-[13px] text-slate-600 hover:bg-slate-50 hover:text-slate-800 transition-colors">
                                <i class="fas fa-home w-4 text-slate-400 text-xs"></i> لوحة التحكم
                            </a>
                            <a href="<?php echo e(route('admin.profile')); ?>" class="flex items-center gap-3 px-4 py-2.5 text-[13px] text-slate-600 hover:bg-slate-50 hover:text-slate-800 transition-colors">
                                <i class="fas fa-user w-4 text-slate-400 text-xs"></i> الملف الشخصي
                            </a>
                            <a href="<?php echo e(route('settings')); ?>" class="flex items-center gap-3 px-4 py-2.5 text-[13px] text-slate-600 hover:bg-slate-50 hover:text-slate-800 transition-colors">
