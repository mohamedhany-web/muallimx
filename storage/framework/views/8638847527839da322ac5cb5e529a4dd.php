...
">
    <div class="flex min-h-screen">
        ...
        <aside class="flex flex-col w-64 fixed inset-y-0 z-50 bg-slate-900 border-<?php echo e($rtl ? 'l' : 'r'); ?> border-slate-700/50 transition-transform duration-300 ease-out <?php echo e($rtl ? 'right-0' : 'left-0'); ?>"
            ...
            <div class="p-4 border-b border-slate-700/50 flex items-center justify-between">
                <a href="<?php echo e(route('community.dashboard')); ?>" class="flex items-center gap-2 text-white font-bold" @click="if(!isLg) sidebarOpen = false">
            ...
            <nav class="flex-1 p-3 space-y-0.5 overflow-y-auto">
                <a href="<?php echo e(route('community.dashboard')); ?>" @click="if(!isLg) sidebarOpen = false" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-300 hover:bg-slate-800 hover:text-white <?php echo e(request()->routeIs('community.dashboard') ? 'bg-slate-700/80 text-white' : ''); ?>">
                    <i class="fas fa-home w-5"></i>
                    <span><?php echo e(__('auth.home')); ?></span>
                </a>
                <a href="<?php echo e(route('community.competitions.index')); ?>" @click="if(!isLg) sidebarOpen = false" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-300 hover:bg-slate-800 hover:text-white <?php echo e(request()->routeIs('community.competitions.*') ? 'bg-slate-700/80 text-white' : ''); ?>">
                    <i class="fas fa-trophy w-5"></i>
                    <span><?php echo e(__('admin.community_competitions')); ?></span>
                </a>
                <a href="<?php echo e(route('community.datasets.index')); ?>" @click="if(!isLg) sidebarOpen = false" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-300 hover:bg-slate-800 hover:text-white <?php echo e(request()->routeIs('community.datasets.*') ? 'bg-slate-700/80 text-white' : ''); ?>">
                    <i class="fas fa-database w-5"></i>
                    <span><?php echo e(__('admin.community_datasets')); ?></span>
                </a>
                <a href="<?php echo e(route('community.discussions.index')); ?>" @click="if(!isLg) sidebarOpen = false" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-300 hover:bg-slate-800 hover:text-white <?php echo e(request()->routeIs('community.discussions.*') ? 'bg-slate-700/80 text-white' : ''); ?>">
            ...
            <div class="p-3 border-t border-slate-700/50">
                <a href="<?php echo e(route('dashboard')); ?>" @click="if(!isLg) sidebarOpen = false" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-400 hover:bg-slate-800 hover:text-white text-sm">
                ...
                <div x-data="{ open: false }" class="relative">
                    <button @click="open = !open" class="flex items-center gap-2 p-2 rounded-xl hover:bg-slate-100">
                        <?php if(auth()->user()->profile_image ?? null): ?>
                            <img src="<?php echo e(auth()->user()->profile_image_url ?? '#'); ?>" alt="" class="w-8 h-8 rounded-full object-cover">
                        <?php else: ?>
                            <span class="w-8 h-8 rounded-full bg-cyan-600 text-white flex items-center justify-center font-bold text-sm"><?php echo e(mb_substr(auth()->user()->name ?? 'م', 0, 1)); ?></span>
                        <?php endif; ?>
                        <span class="hidden sm:inline font-semibold text-slate-700"><?php echo e(auth()->user()->name); ?></span>
                        <i class="fas fa-chevron-down text-slate-500 text-xs"></i>
                    </button>
                    <div x-show="open" @click.outside="open = false" x-transition class="absolute top-full mt-1 <?php echo e($rtl ? 'right-0' : 'left-0'); ?> w-48 py-1 bg-white rounded-xl shadow-xl border border-slate-200">
                        <a href="<?php echo e(route('community.dashboard')); ?>" class="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-50"><?php echo e(__('auth.dashboard')); ?></a>
                        <a href="<?php echo e(route('dashboard')); ?>" class="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-50">المنصة الرئيسية</a>
                        <form method="POST" action="<?php echo e(route('logout')); ?>">
                            <?php echo csrf_field(); ?>
                            <button type="submit" class="block w-full text-right px-4 py-2 text-sm text-red-600 hover:bg-slate-50"><?php echo e(__('auth.logout')); ?></button>
                        </form>
                    </div>
                </div>
            </header>
