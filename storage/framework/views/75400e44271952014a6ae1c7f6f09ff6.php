...
<body class="bg-white text-navy-950 antialiased font-body"
    ...
    <main class="flex-1">
        ...
        <section class="relative min-h-[70vh] flex items-center overflow-hidden bg-navy-950 noise">
            ...
            <div class="absolute bottom-[-10%] <?php echo e($isRtl?'right-[-5%]':'left-[-5%]'); ?> w-[500px] h-[500px] rounded-full bg-blue-600/8 blur-[100px]"></div>
                ...
                <div class="text-center max-w-4xl mx-auto">
                    <div class="reveal">
                        <span class="inline-flex items-center gap-2.5 px-5 py-2 rounded-full bg-white/[0.06] border border-white/[0.1] text-brand-300 text-sm font-medium backdrop-blur-sm">
                            ...
                            <?php echo e(__('public.courses_page_title')); ?>
                    ...
                    <h1 class="reveal stagger-1 font-heading text-4xl sm:text-5xl md:text-6xl font-black leading-[1.15] text-white mt-6">
                        ...
                        <br>
                        <span class="text-gradient"><?php echo e(__('public.courses_hero_highlight')); ?></span>
                    </h1>
                        ...
                        <div class="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center bg-white/[0.05] border border-white/[0.1] rounded-2xl p-3 backdrop-blur-sm">
                            ...
                            <div class="relative min-w-[160px]">
                                <select x-model="selectedLevel"
                                        class="w-full appearance-none bg-white/[0.05] border border-white/[0.08] rounded-xl px-4 py-3 <?php echo e($isRtl?'pl-10':'pr-10'); ?> text-white text-sm sm:text-base focus:outline-none focus:border-brand-500/50">
                                ...
                                <div class="absolute <?php echo e($isRtl?'left':'right'); ?>-3 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400">
                                    <i class="fas fa-chevron-down text-xs"></i>
                                </div>
                            </div>
                        </div>
                        <p x-show="searchQuery || selectedLevel" x-cloak x-transition class="mt-3 text-sm text-slate-400">
                            <span x-text="filteredCourses.length"></span> <?php echo e(__('public.course_available')); ?>
                    ...
                    <div class="reveal stagger-4 flex flex-wrap justify-center gap-6 mt-10">
                        <div class="flex items-center gap-2 text-white/70 text-sm">
                            ...
                            <span><span class="font-bold text-white" x-text="courses.length">0</span> <?php echo e(__('public.course_available')); ?></span>
                        </div>
                        <div class="flex items-center gap-2 text-white/70 text-sm">
        ...
        <section id="courses" class="py-20 md:py-28 bg-white">
            <div class="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
                <div class="text-center max-w-3xl mx-auto mb-14 reveal">
                    <span class="inline-block px-4 py-1.5 rounded-full bg-brand-50 text-brand-600 text-sm font-semibold mb-4"><?php echo e(__('public.courses_section_title')); ?></span>
                    <h2 class="font-heading text-3xl sm:text-4xl md:text-5xl font-black text-navy-950 mb-5 leading-tight">
                        <?php echo e(__('public.courses_section_title')); ?>

                        <span class="text-gradient"><?php echo e(__('public.courses_hero_highlight')); ?></span>
                    </h2>
                    <p class="text-lg text-slate-500 leading-relaxed"><?php echo e(__('public.courses_section_subtitle')); ?></p>
                </div>
                ...
                <template x-if="filteredCourses && filteredCourses.length > 0">
                    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 lg:gap-8">
                        <template x-for="(course, index) in filteredCourses" :key="course.id">
                            <a :href="'<?php echo e(url('/course')); ?>/' + course.id"
                               ...
                               :class="'stagger-' + (Math.min(index + 1, 4))">
                                ...
                                <div class="relative aspect-[16/10] bg-gradient-to-br from-brand-500 via-blue-500 to-navy-700 overflow-hidden">
                                    <template x-if="course.thumbnail">
                                        <img :src="'<?php echo e(asset('storage')); ?>/' + course.thumbnail" :alt="course.title"
                                    ...
                                    <template x-if="!course.thumbnail">
                                        <div class="absolute inset-0 flex items-center justify-center">
                                            <div class="w-20 h-20 rounded-full bg-white/10 flex items-center justify-center backdrop-blur-sm">
                                                <i class="fas fa-play text-white/80 text-2xl <?php echo e($isRtl?'mr-[-2px]':'ml-1'); ?>"></i>
                                            </div>
                                        </div>
                                    </template>
                                    ...
                                    <template x-if="course.is_featured">
                                        <span class="absolute top-3 <?php echo e($isRtl?'right':'left'); ?>-3 px-3 py-1.5 rounded-full bg-amber-400/95 text-amber-900 text-[11px] font-bold flex items-center gap-1.5 shadow-lg shadow-amber-500/20">
                                    ...
                                    <div class="absolute top-3 <?php echo e($isRtl?'left':'right'); ?>-3">
                                        <span class="px-2.5 py-1 rounded-lg text-[11px] font-bold backdrop-blur-md shadow-sm"
                                              ...
                                              x-text="course.level === 'beginner' ? '<?php echo e(__('public.level_beginner')); ?>' : (course.level === 'intermediate' ? '<?php echo e(__('public.level_intermediate')); ?>' : '<?php echo e(__('public.level_advanced')); ?>')">
                                        </span>
                                    </div>
                                    ...
                                    <div class="absolute bottom-0 inset-x-0 p-4 flex items-end justify-between">
                                        <div class="flex items-center gap-3">
                                            <span class="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-white/95 backdrop-blur text-navy-800 text-[11px] font-bold shadow-sm">
                                                ...
                                                <span x-text="(course.lessons_count || 0) + ' <?php echo e(__('public.lesson_single')); ?>'"></span>
                                            </span>
                                            <template x-if="course.duration_hours && course.duration_hours > 0">
                                                <span class="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-white/95 backdrop-blur text-navy-800 text-[11px] font-bold shadow-sm">
                                                    <i class="fas fa-clock text-blue-500 text-[10px]"></i>
                                                    <span x-text="course.duration_hours + ' <?php echo e(__('public.hours')); ?>'"></span>
                                                </span>
                                            </template>
                                        </div>
                                    </div>
                                ...
                                <div class="p-5 sm:p-6 flex flex-col flex-1">
                                    ...
                                    <h3 class="font-heading text-lg sm:text-xl font-bold text-navy-950 mb-2.5 line-clamp-2 leading-snug group-hover:text-brand-600 transition-colors duration-300"
                                        x-text="course.title || '<?php echo e(addslashes(__('public.no_title_fallback'))); ?>'"></h3>
                                    ...
                                    <template x-if="course.instructor && course.instructor.name">
                                        <div class="flex items-center gap-2.5 mb-4 pb-4 border-b border-slate-100">
                                            ...
                                            <div class="min-w-0">
                                                <p class="text-[11px] text-slate-400 leading-none mb-0.5"><?php echo e(__('public.instructor_label')); ?></p>
                                    ...
                                    <div class="flex items-center justify-between mt-auto">
                                        <div>
                                            <template x-if="course.price && course.price > 0">
                                                <div>
                                                    ...
                                                    <span class="text-xs text-slate-400 font-medium"><?php echo e(__('public.currency_egp')); ?></span>
                                                </div>
                                            </template>
                                            <template x-if="!course.price || course.price == 0">
                                        ...
                                        <span class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-<?php echo e($isRtl?'r':'l'); ?> from-brand-500 to-brand-600 text-white font-bold text-[13px] shadow-lg shadow-brand-600/20 group-hover:shadow-brand-600/40 group-hover:scale-105 transition-all duration-300">
                                            <?php echo e(__('public.view_details')); ?>

                                            <i class="fas fa-arrow-<?php echo e($isRtl?'left':'right'); ?> text-[10px]"></i>
                                        </span>
                                    </div>
                                </div>
                            </a>
                ...
                <div x-show="filteredCourses && filteredCourses.length === 0" x-cloak x-transition class="text-center py-20 reveal">
                    <div class="max-w-md mx-auto">
                        ...
                        <h3 class="font-heading text-xl font-bold text-navy-950 mb-2"><?php echo e(__('public.no_results')); ?></h3>
                ...
                <div class="text-center py-20 reveal">
                    <div class="max-w-md mx-auto">
                        ...
                        <h3 class="font-heading text-2xl font-bold text-navy-950 mb-3"><?php echo e(__('public.coming_soon')); ?></h3>
                        <p class="text-slate-500 mb-8 leading-relaxed"><?php echo e(__('public.coming_soon_courses')); ?></p>
                        <a href="<?php echo e(route('register')); ?>" class="btn-primary inline-flex items-center gap-2.5 bg-gradient-to-l from-brand-500 to-brand-600 text-white px-7 py-3.5 rounded-2xl font-bold shadow-xl shadow-brand-600/25">
                            <i class="fas fa-bell"></i>
                            <?php echo e(__('public.subscribe_updates')); ?>
        ...
        <section class="py-20 md:py-28 bg-slate-50/50">
            <div class="max-w-4xl mx-auto px-5 sm:px-8 lg:px-12 text-center reveal">
                ...
                <div class="flex flex-col sm:flex-row gap-4 justify-center">
                    ...
                    <a href="<?php echo e(route('login')); ?>" class="btn-outline inline-flex items-center justify-center gap-3 bg-white border-2 border-slate-200 hover:border-brand-300 text-navy-950 font-semibold text-base sm:text-lg px-8 py-4 rounded-2xl">
                        <?php echo e(__('public.have_account')); ?>
