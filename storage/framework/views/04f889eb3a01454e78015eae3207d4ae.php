...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <a href="<?php echo e(route('instructor.live-sessions.create')); ?>" class="inline-flex items-center gap-2 px-5 py-2.5 bg-red-500 hover:bg-red-600 text-white rounded-xl font-semibold shadow-lg shadow-red-500/25 transition-all">
    ...
    <div class="flex gap-2 flex-wrap">
        <a href="<?php echo e(route('instructor.live-sessions.index')); ?>" class="px-3 py-1.5 rounded-lg text-sm <?php echo e(!request('status') ? 'bg-slate-800 dark:bg-white text-white dark:text-slate-800 font-semibold' : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-200'); ?> transition-colors">الكل</a>
        <a href="<?php echo e(route('instructor.live-sessions.index', ['status' => 'live'])); ?>" class="px-3 py-1.5 rounded-lg text-sm <?php echo e(request('status') === 'live' ? 'bg-red-500 text-white font-semibold' : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-200'); ?> transition-colors">مباشر</a>
        <a href="<?php echo e(route('instructor.live-sessions.index', ['status' => 'scheduled'])); ?>" class="px-3 py-1.5 rounded-lg text-sm <?php echo e(request('status') === 'scheduled' ? 'bg-blue-500 text-white font-semibold' : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-200'); ?> transition-colors">مجدولة</a>
    ...
    <div class="space-y-3">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl border <?php echo e($session->isLive() ? 'border-red-300 dark:border-red-800 ring-1 ring-red-200 dark:ring-red-900/50' : 'border-slate-200 dark:border-slate-700'); ?> p-5 flex flex-col sm:flex-row sm:items-center gap-4">
            ...
            <div class="flex items-center gap-2 flex-shrink-0">
                <?php if($session->isLive()): ?>
                    <a href="<?php echo e(route('instructor.live-sessions.room', $session)); ?>" class="px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-lg text-sm font-semibold shadow-lg shadow-red-500/25 transition-all">
        ...
        <div class="text-center py-16 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
            ...
            <a href="<?php echo e(route('instructor.live-sessions.create')); ?>" class="inline-flex items-center gap-2 px-5 py-2.5 bg-red-500 text-white rounded-xl font-semibold hover:bg-red-600 transition-colors">
