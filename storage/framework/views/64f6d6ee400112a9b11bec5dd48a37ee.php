    ...
    <div class="rounded-2xl bg-white dark:bg-slate-800/95 border border-slate-200 dark:border-slate-700 shadow-lg overflow-hidden">
        <div class="bg-gradient-to-l from-sky-50 via-white to-white dark:from-slate-800/90 dark:via-slate-800/95 dark:to-slate-900/95 p-6 border-b border-slate-100 dark:border-slate-700">
            ...
            <p class="text-sm text-slate-600 dark:text-slate-400">
                مدة الباقة: <strong><?php echo e($durationLabel); ?></strong>
                <?php if($currentKey === 'teacher_free'): ?>
                    · تنتهي تلقائياً في <strong><?php echo e($sub->end_date?->format('Y-m-d')); ?></strong> ولا تتجدد
                <?php else: ?>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
            <div class="p-4 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50/50 dark:bg-slate-700/40">
                ...
                <p class="mt-1 font-bold text-slate-900 dark:text-slate-100">
                    <?php echo e($classroomUsed); ?> / <?php echo e($classroomCap >= 9999 ? '∞' : $classroomCap); ?>

                </p>
            </div>
            <div class="p-4 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50/50 dark:bg-slate-700/40">
                <p class="text-xs font-semibold text-slate-500 dark:text-slate-400">أقصى مدة للاجتماع</p>
                <p class="mt-1 font-bold text-slate-900 dark:text-slate-100"><?php echo e((int) ($limits['classroom_max_duration_minutes'] ?? 0)); ?> دقيقة</p>
            </div>
            <div class="p-4 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50/50 dark:bg-slate-700/40">
                <p class="text-xs font-semibold text-slate-500 dark:text-slate-400">أقصى مشاركين</p>
                <p class="mt-1 font-bold text-slate-900 dark:text-slate-100"><?php echo e((int) ($limits['classroom_max_participants'] ?? 0)); ?></p>
            </div>
            <div class="p-4 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50/50 dark:bg-slate-700/40">
                <p class="text-xs font-semibold text-slate-500 dark:text-slate-400">أقسام التسويق الشخصي</p>
                <p class="mt-1 font-bold text-slate-900 dark:text-slate-100"><?php echo e((int) ($limits['personal_marketing_profile_sections'] ?? 0)); ?></p>
            </div>
        </div>
                        ...
                        <div class="flex items-center justify-between gap-3">
                            <div>
                                <div class="text-sm font-black text-slate-900 dark:text-slate-100">
                                    <?php if($planKey === 'teacher_pro'): ?>
                                        الباقة الشاملة
                                    <?php elseif($planKey === 'teacher_starter'): ?>
                                        الباقة الأساسية
                                    <?php else: ?>
                                        الباقة المجانية
                                    <?php endif; ?>
