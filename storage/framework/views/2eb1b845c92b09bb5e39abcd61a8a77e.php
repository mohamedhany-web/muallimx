        ...
        <div class="relative flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            <div>
                <div class="flex items-center gap-3 flex-wrap">
                    <h1 class="text-3xl font-black tracking-tight">خطة <?php echo e($subscription->plan_name); ?></h1>
                    <span class="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold <?php echo e($statusBadge[$subscription->status] ?? 'bg-white/20 text-white'); ?>">
                        <span class="w-2 h-2 rounded-full <?php echo e($subscription->status === 'active' ? 'bg-emerald-300' : ($subscription->status === 'expired' ? 'bg-rose-300' : 'bg-amber-300')); ?>"></span>
                ...
                <p class="mt-3 text-white/70 max-w-2xl">
                    نوع الاشتراك: <?php echo e(\App\Models\Subscription::typeLabel($subscription->subscription_type)); ?> · دورة الفوترة: <?php echo e(\App\Models\Subscription::billingCycleLabel($subscription->billing_cycle)); ?>
                ...
                <div class="mt-6 flex flex-wrap items-center gap-4 text-xs font-semibold">
                    <span class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/15">
                        ...
                        <?php echo e($subscription->user->name ?? 'غير معروف'); ?>

                    </span>
                    <span class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/15">
                        <i class="fas fa-phone text-xs"></i>
                        <?php echo e($subscription->user->phone ?? 'غير متوفر'); ?>

                    </span>
                    <span class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/15">
                        <i class="fas fa-calendar-alt text-xs"></i>
                        <?php echo e(optional($subscription->created_at)->diffForHumans()); ?>

                    </span>
                </div>
            </div>
            <div class="flex flex-wrap gap-3 justify-end">
                <a href="<?php echo e(route('admin.subscriptions.edit', $subscription)); ?>" class="inline-flex items-center gap-2 px-5 py-3 rounded-2xl bg-white text-sky-700 font-semibold shadow-lg hover:shadow-xl transition-all">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                    <div>
                        ...
                        <p class="mt-2 text-base font-semibold text-gray-900"><?php echo e(\App\Models\Subscription::typeLabel($subscription->subscription_type)); ?></p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="mt-2 text-2xl font-black text-gray-900"><?php echo e(number_format($subscription->price, 2)); ?> ج.م</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 uppercase">حالة التجديد التلقائي</p>
                        <p class="mt-2">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium <?php echo e($subscription->auto_renew ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-700'); ?>">
                                <?php echo e($subscription->auto_renew ? 'مفعل' : 'غير مفعل'); ?>
            ...
            <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm">
                    ...
                    <div class="p-4 rounded-2xl border border-rose-100 bg-rose-50/80">
                        ...
                        <p class="mt-2 text-base font-bold text-gray-900"><?php echo e($subscription->end_date?->format('Y-m-d') ?? 'غير متوفر'); ?></p>
                    </div>
                </div>
                ...
                <div class="relative">
                    ...
                    <div class="relative flex justify-between text-xs text-gray-500">
                        <span><?php echo e($subscription->start_date?->diffForHumans() ?? '—'); ?></span>
                        <span><?php echo e($subscription->end_date?->diffForHumans() ?? '—'); ?></span>
                    </div>
                </div>
            </div>
            ...
            <?php if($subscription->invoice): ?>
                <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
                    ...
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            <p class="text-sm font-semibold text-gray-900">فاتورة رقم <?php echo e($subscription->invoice->invoice_number); ?></p>
                            <p class="text-xs text-gray-500 mt-1">أُنشئت بتاريخ <?php echo e(optional($subscription->invoice->created_at)->format('Y-m-d')); ?></p>
                        </div>
                        <a href="<?php echo e(route('admin.invoices.show', $subscription->invoice)); ?>" class="inline-flex items-center gap-2 px-5 py-3 rounded-2xl bg-sky-100 text-sky-600 font-semibold hover:bg-sky-200 transition-all">
        ...
        <div class="space-y-6">
            <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
                ...
                <div class="space-y-4 text-sm">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-base font-semibold text-gray-900"><?php echo e($subscription->user->phone ?? 'غير متوفر'); ?></p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 uppercase">البريد الإلكتروني</p>
                        <p class="mt-1 text-base font-semibold text-gray-900"><?php echo e($subscription->user->email ?? 'غير متوفر'); ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
                ...
                <div class="space-y-3">
                    <a href="<?php echo e(route('admin.subscriptions.edit', $subscription)); ?>" class="flex items-center justify-between px-4 py-3 rounded-2xl border border-sky-100 bg-sky-50/70 text-sky-600 hover:border-sky-200 transition-all">
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\Muallimx\resources\views\admin\subscriptions\show.blade.php ENDPATH**/ ?>