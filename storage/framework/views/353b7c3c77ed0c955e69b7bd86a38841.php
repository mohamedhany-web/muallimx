...
<?php $__env->startSection('title', 'إدارة الرسائل - Mindlytics'); ?>
<?php $__env->startSection('header', 'إدارة الرسائل'); ?>
...
<div class="space-y-6 sm:space-y-10">
    ...
    <section class="rounded-3xl bg-white/95 backdrop-blur border border-slate-200 shadow-lg overflow-hidden">
        <div class="px-5 py-6 sm:px-8 lg:px-12 border-b border-slate-200 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-2xl font-bold text-slate-900">لوحة إدارة الرسائل</h2>
                <p class="text-sm text-slate-500 mt-2">إدارة وإرسال الرسائل والتقارير عبر الواتساب</p>
            </div>
            ...
            <form method="GET" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <select name="status" class="w-full rounded-2xl border border-slate-200 bg-white/80 px-4 py-2.5 text-sm text-slate-900 focus:ring-2 focus:ring-sky-500 focus:border-sky-400 transition-all">
                        ...
                        <option value="failed" <?php echo e(request('status') === 'failed' ? 'selected' : ''); ?>>فشل الإرسال</option>
                    </select>
                </div>
                <div>
                    ...
                    <select name="type" class="w-full rounded-2xl border border-slate-200 bg-white/80 px-4 py-2.5 text-sm text-slate-900 focus:ring-2 focus:ring-sky-500 focus:border-sky-400 transition-all">
                        ...
                        <option value="text" <?php echo e(request('type') === 'text' ? 'selected' : ''); ?>>رسالة نصية</option>
                        <option value="template" <?php echo e(request('type') === 'template' ? 'selected' : ''); ?>>قالب</option>
                    </select>
                </div>
                                    ...
                                    <div class="flex items-center gap-4">
                                        <div class="w-12 h-12 bg-gradient-to-br from-sky-500 to-blue-600 rounded-xl flex items-center justify-center shadow-md">
                                            <span class="text-white font-bold text-lg">
                                                <?php echo e($message->user ? mb_substr($message->user->name, 0, 1, 'UTF-8') : 'غ'); ?>

                                            </span>
                                        </div>
                                        <div>
                                            <div class="text-sm font-semibold text-slate-900">
                                                <?php echo e($message->user->name ?? 'غير معروف'); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\Muallimx\resources\views\admin\messages\index.blade.php ENDPATH**/ ?>